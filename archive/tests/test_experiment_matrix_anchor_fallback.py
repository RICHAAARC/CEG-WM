...
def test_extract_hf_truncation_comparison_reads_baseline_trace(tmp_path: Path) -> None:
    """
    功能：验证 HF 截断 baseline 比较提取会读取 baseline 分数与 trace。

    Verify HF truncation baseline comparison extraction consumes baseline score and trace metadata.
    ...
    payload = experiment_matrix._extract_hf_truncation_baseline_comparison_from_detect_record(run_root)
    assert payload.get("comparison_ready") is True
    assert payload.get("comparison_source") == "real_hf_truncation_baseline_record"
