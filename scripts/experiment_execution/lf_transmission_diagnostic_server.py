"""Colab/server launcher for the frozen LF transport diagnostic."""
...
class LfTransmissionServerError(RuntimeError):
    """The server could not start or package the LF transport worker."""


def execute_lf_transmission_diagnostic_server_session(
    *,
    ...
    if (
        ...
        raise LfTransmissionServerError("execution roots must be disjoint")
    if (
        SAFE_ID_PATTERN.fullmatch(run_id) is None
        or SAFE_ID_PATTERN.fullmatch(session_id) is None
    ):
        raise LfTransmissionServerError("run or session identity is invalid")
    ...
    if not hf_token or not root_key:
        raise LfTransmissionServerError("HF_TOKEN and CEG_WM_ROOT_KEY are required")
    ...
    exit_code, worker = execute_lf_transmission_diagnostic_session(
        repository_root=repository,
...
def _main() -> int:
    ...
    exit_code, result = execute_lf_transmission_diagnostic_server_session(
