"""Colab/server launcher for HF-only detector directional validation."""
...
class HfDetectorDirectionalServerError(RuntimeError):
    """The server could not start the HF detector directional worker."""


def execute_hf_only_detector_directional_validation_server_session(
    *,
    ...
    authorized_scientific_unit_count: int = 8,
    ...
    exit_code, worker = execute_hf_only_detector_directional_validation_session(
        repository_root=repository,
...
def _main() -> int:
    ...
    parser.add_argument("--authorized-scientific-unit-count", required=True, type=int)
    parser.add_argument("--skip-dependency-install", action="store_true")
    args = parser.parse_args()
    exit_code, result = execute_hf_only_detector_directional_validation_server_session(
        ...
        authorized_scientific_unit_count=args.authorized_scientific_unit_count,
