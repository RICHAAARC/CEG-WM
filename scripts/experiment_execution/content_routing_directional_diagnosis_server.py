"""Server launcher for the frozen content-routing directional diagnosis."""
...
def _validated_artifact(
    ...
    *,
    ...
    if type(value) is not str:
        raise ContentRoutingDirectionalServerError("worker returned no artifact")
    ...
    if (
        ...
        raise ContentRoutingDirectionalServerError("worker artifact is invalid")
    return artifact


def execute_content_routing_directional_diagnosis_server_session(
    *,
    ...
    if (
        ...
        raise ContentRoutingDirectionalServerError("execution roots must be disjoint")
    if (
        SAFE_ID_PATTERN.fullmatch(run_id) is None
        or SAFE_ID_PATTERN.fullmatch(session_id) is None
    ):
        raise ContentRoutingDirectionalServerError("run or session identity is invalid")
    ...
    if run_id != protocol.run_id:
        raise ContentRoutingDirectionalServerError("run identity drifted")
    ...
    if (
        ...
        raise ContentRoutingDirectionalServerError("worker frozen identity drifted")
    receipt_path = (
        persistent
        / run_id
        / "server_receipts"
