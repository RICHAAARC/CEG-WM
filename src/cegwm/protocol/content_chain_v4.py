"""Frozen loader for the clean Content V4 whitened-LF evaluation protocol."""
...
from cegwm.protocol.content_chain_v2 import (
    _CONTENT_ANALYSIS,
    _DETECTION_ACCESS as _V2_DETECTION_ACCESS,
    _EXPECTED_ROSTER,
    _freeze,
    _load_roster,
...
CONTENT_V4_PROTOCOL_ID = "cegwm-stage-a-content-v4-whitened-lf-adaptive-hf-clean-v1"
CONTENT_V4_EXECUTION_SCOPE_ID = (
    "content_v4_whitened_lf_adaptive_hf_engineering_and_stage_a_evaluation_v1"
)
CONTENT_V4_RECORD_CONTRACT_ID = "content_v4_whitened_lf_adaptive_hf_record_v1"
CONTENT_V4_STATE_SCHEMA_ID = "content_v4_resumable_state_v1"
CONTENT_V4_RUN_PREFIX = "content-v4"
CONTENT_V4_ARMS = (
    CONTENT_V4_EVALUATED_CANDIDATE_ID,
    f"primary_null__{CONTENT_V4_EVALUATED_CANDIDATE_ID}",
)
...
_LF_DETECTION_OPERATOR = {
    "scorer_id": CONTENT_V4_LF_SCORER_ID,
...
_DETECTION_ACCESS = {
    **_V2_DETECTION_ACCESS,
    "lf_detector": CONTENT_V4_LF_SCORER_ID,
}
_EXECUTION_FLOW = {
    "roster_manifest": "content_adaptive_dual_branch_v2_clean.jsonl",
    "formal_roster_sha256": CONTENT_V4_ROSTER_SHA256,
    "split": "content_adaptive_dual_branch_v2_clean_v1",
    "fixed_units": 8,
    "record_arms_in_order": list(CONTENT_V4_ARMS),
...
def _validate_config(config: Mapping[str, Any]) -> None:
    if config.get("protocol_version") != 1 or config.get("protocol_id") != CONTENT_V4_PROTOCOL_ID:
        raise ValueError("unexpected Content V4 protocol identity")
    if config.get("execution_scope_id") != CONTENT_V4_EXECUTION_SCOPE_ID:
        raise ValueError("unexpected Content V4 execution scope")
    if config.get("scientific_status") != "not_evaluated_until_complete_real_gpu_rc0":
        raise ValueError("Content V4 protocol cannot preclaim scientific evidence")
    expected_sections = {
    ...
    for name, expected in expected_sections.items():
        if _mapping(config, name) != expected:
            raise ValueError(f"Content V4 {name.replace('_', ' ')} differs")
    if config.get("limitations") != _LIMITATIONS:
        raise ValueError("Content V4 limitations differ")
    if set(config) != {
        "protocol_version", "protocol_id", "execution_scope_id", "scientific_status",
        *expected_sections, "limitations",
    }:
        raise ValueError("Content V4 config fields differ")


def load_content_v4_clean_protocol(
    config_path: str | Path,
    roster_path: str | Path,
) -> ContentChainProtocol:
    """Load Content V4 and bind it to the unchanged ordered eight-unit roster."""
    ...
    if not isinstance(config, dict):
        raise ValueError("Content V4 config must be an object")
    _validate_config(config)
    roster_bytes = roster_path.read_bytes()
    if hashlib.sha256(roster_bytes).hexdigest() != CONTENT_V4_ROSTER_SHA256:
        raise ValueError("Content V4 formal roster SHA differs")
    ...
    if received != _EXPECTED_ROSTER:
        raise ValueError("Content V4 ordered roster differs")
    ...
    if protocol_digest != CONTENT_V4_PROTOCOL_DIGEST:
        raise ValueError("Content V4 canonical protocol digest differs")
