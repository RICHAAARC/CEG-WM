"""Frozen Phase-1 contract for content calibrated weighted joint scoring."""
...
CONTENT_CALIBRATION_BASE_EXACT = "49bb03ed697a47048f5730ecfc85a9f29cb0b58a"
CONTENT_CALIBRATION_METHOD_ID = "content_v9_v6_calibrated_weighted_joint_v1"
...
CONTENT_CALIBRATION_CALIBRATION_PROTOCOL_DIGEST = (
    "68f37585eb6eab123bad7c1703767df08404718ce4771f73fbbec236491a1e01"
)
CONTENT_CALIBRATION_FUTURE_EVALUATION_PROTOCOL_ID = (
    "cegwm-stage-a-content-v9-calibrated-weighted-joint-clean-v1"
)
CONTENT_CALIBRATION_RECORD_CONTRACT_ID = "content_v9_calibrated_weighted_joint_record_v1"
CONTENT_CALIBRATION_STATE_SCHEMA_ID = "content_v9_resumable_state_v1"
CONTENT_CALIBRATION_RUN_PREFIX = "content-v9"
CONTENT_CALIBRATION_CALIBRATION_RECEIPT_ID = "content_v9_calibration_receipt_v1"
CONTENT_CALIBRATION_CALIBRATION_SPLIT = "content_v9_calibration_v1"
CONTENT_CALIBRATION_EVALUATION_SPLIT = "content_v9_clean_evaluation_v1"
CONTENT_CALIBRATION_CALIBRATION_MANIFEST = "content_v9_calibration_v1.jsonl"
CONTENT_CALIBRATION_EVALUATION_MANIFEST = "content_v9_clean_evaluation_v1.jsonl"
...
CONTENT_CALIBRATION_EVALUATION_MANIFEST_SHA256 = (
    "dd9d9c60974f07f3727b0c46b08c1678dfb9b57339a662735ad4178b9473849d"
)
CONTENT_CALIBRATION_EVALUATION_PROMPT_LIST_SHA256 = (
    "ed8796c183200ec826023d44f9ebe06293968f5a387dbcf242addaf036e46617"
)
CONTENT_CALIBRATION_CALIBRATION_COUNT = 32
CONTENT_CALIBRATION_EVALUATION_COUNT = 8
CONTENT_CALIBRATION_PAIRED_NULL_SCORE_COUNT = 1056
CONTENT_CALIBRATION_CALIBRATION_KEY_DOMAIN = (
    "stage-a/content-v9-calibrated-weighted-joint-calibration-key/v1"
)
CONTENT_CALIBRATION_WRONG_KEY_DOMAIN = "stage-a/content-adaptive-v2-external-wrong-key/v1"
CONTENT_CALIBRATION_CALIBRATION_ASSET_SCHEMA_ID = (
    "cegwm_content_v9_calibrated_weighted_joint_asset_v1"
)
CONTENT_CALIBRATION_CALIBRATION_ASSET_ROLE_ID = "content_v9_calibrated_weighted_joint_v1"
...
@dataclass(frozen=True, slots=True)
class ContentCalibrationContract:
    calibration: tuple[ContentCalibrationUnit, ...]
    evaluation: tuple[ContentCalibrationUnit, ...]
...
def _load_manifest(
    ...
    *,
    ...
    unit_prefix: str,
    source_prefix: str,
) -> tuple[ContentCalibrationUnit, ...]:
    raw = path.read_bytes()
    if hashlib.sha256(raw).hexdigest() != manifest_sha256 or not raw.endswith(b"\n"):
        raise ValueError("content manifest bytes differ")
    lines = raw.splitlines()
    if len(lines) != count:
        raise ValueError("content manifest count differs")
    units: list[ContentCalibrationUnit] = []
    for ordinal, line in enumerate(lines, 1):
        try:
            value = json.loads(line)
        except (UnicodeDecodeError, json.JSONDecodeError) as error:
            raise ValueError("content manifest must be UTF-8 JSONL") from error
        if not isinstance(value, dict) or tuple(value) != _FIELDS or _stable_line(value) != line:
            raise ValueError("content manifest fields, order, or encoding differ")
        expected = {
        ...
        if any(value.get(name) != expected_value for name, expected_value in expected.items()):
            raise ValueError("content ordered unit identity differs")
        if not isinstance(value["prompt"], str) or not value["prompt"].strip():
            raise ValueError("content prompt must be non-empty text")
        units.append(ContentCalibrationUnit(**value))
    prompt_bytes = b"".join(unit.prompt.encode("utf-8") + b"\n" for unit in units)
    if hashlib.sha256(prompt_bytes).hexdigest() != prompt_sha256:
        raise ValueError("content ordered prompt identity differs")
    return tuple(units)


def _identity_sets(units: Iterable[ContentCalibrationUnit]) -> tuple[set[Any], ...]:
...
def _require_unique_disjoint(
    calibration: tuple[ContentCalibrationUnit, ...], evaluation: tuple[ContentCalibrationUnit, ...]
) -> None:
    calibration_sets = _identity_sets(calibration)
    evaluation_sets = _identity_sets(evaluation)
    if any(len(values) != CONTENT_CALIBRATION_CALIBRATION_COUNT for values in calibration_sets):
        raise ValueError("content calibration identities must be unique")
    if any(len(values) != CONTENT_CALIBRATION_EVALUATION_COUNT for values in evaluation_sets):
        raise ValueError("content evaluation identities must be unique")
    if any(left & right for left, right in zip(calibration_sets, evaluation_sets, strict=True)):
        raise ValueError("content calibration and evaluation identities overlap")
...
def _expected_config() -> dict[str, Any]:
    return {
        "asset_contract": {
            ...
            "asset_schema_id": CONTENT_CALIBRATION_CALIBRATION_ASSET_SCHEMA_ID,
        ...
        "calibration": {
            ...
            "key_domain": CONTENT_CALIBRATION_CALIBRATION_KEY_DOMAIN,
            "manifest": CONTENT_CALIBRATION_CALIBRATION_MANIFEST,
            "manifest_sha256": CONTENT_CALIBRATION_CALIBRATION_MANIFEST_SHA256,
            "ordered_pair_count": CONTENT_CALIBRATION_PAIRED_NULL_SCORE_COUNT,
            ...
            "prompt_list_sha256": CONTENT_CALIBRATION_CALIBRATION_PROMPT_LIST_SHA256,
            "units": CONTENT_CALIBRATION_CALIBRATION_COUNT,
        },
        "future_evaluation": {
            "formal_fpr_claim": False,
            "manifest": CONTENT_CALIBRATION_EVALUATION_MANIFEST,
            "manifest_sha256": CONTENT_CALIBRATION_EVALUATION_MANIFEST_SHA256,
            "protocol_id": CONTENT_CALIBRATION_FUTURE_EVALUATION_PROTOCOL_ID,
            "prompt_list_sha256": CONTENT_CALIBRATION_EVALUATION_PROMPT_LIST_SHA256,
            "records": 16,
            "units": CONTENT_CALIBRATION_EVALUATION_COUNT,
        },
        "future_read_only_comparison_provenance": {
            ...
            "v6_current_producer_exact": CONTENT_CALIBRATION_BASE_EXACT,
        ...
        "identities": {
            ...
            "record_contract_id": CONTENT_CALIBRATION_RECORD_CONTRACT_ID,
            "state_schema_id": CONTENT_CALIBRATION_STATE_SCHEMA_ID,
        },
        "inherited_v6": {
            "embedding_and_branch_scorers": "byte_semantic_unchanged_from_Content_V6_production",
            "joint_replacement_only": True,
            "wrong_key_domain": CONTENT_CALIBRATION_WRONG_KEY_DOMAIN,
        },
        "joint_operator": {
        ...
        "protocol_id": CONTENT_CALIBRATION_CALIBRATION_PROTOCOL_ID,
        "protocol_version": 1,
...
def load_content_calibration_contract(repo_root: str | Path) -> ContentCalibrationContract:
    root = Path(repo_root) / "configs" / "content_chain"
    calibration = _load_manifest(
        root / CONTENT_CALIBRATION_CALIBRATION_MANIFEST,
        split=CONTENT_CALIBRATION_CALIBRATION_SPLIT,
        count=CONTENT_CALIBRATION_CALIBRATION_COUNT,
        manifest_sha256=CONTENT_CALIBRATION_CALIBRATION_MANIFEST_SHA256,
        prompt_sha256=CONTENT_CALIBRATION_CALIBRATION_PROMPT_LIST_SHA256,
    ...
    evaluation = _load_manifest(
        root / CONTENT_CALIBRATION_EVALUATION_MANIFEST,
        split=CONTENT_CALIBRATION_EVALUATION_SPLIT,
        count=CONTENT_CALIBRATION_EVALUATION_COUNT,
        manifest_sha256=CONTENT_CALIBRATION_EVALUATION_MANIFEST_SHA256,
        prompt_sha256=CONTENT_CALIBRATION_EVALUATION_PROMPT_LIST_SHA256,
    ...
    try:
        ...
        raise ValueError("content phase1 config must be UTF-8 JSON") from error
    if config != _expected_config():
        raise ValueError("content phase1 config differs")
    ...
    if protocol_digest != CONTENT_CALIBRATION_CALIBRATION_PROTOCOL_DIGEST:
        raise ValueError("content calibration canonical protocol digest differs")
    return ContentCalibrationContract(calibration, evaluation, config, protocol_digest)
...
def deterministic_calibration_run_id(protocol_digest: str, public_key_digest: str) -> str:
    ...
    return f"{CONTENT_CALIBRATION_RUN_PREFIX}-calibration-{protocol_digest[:12]}-{public_key_digest[:12]}"


__all__ = [name for name in globals() if name.startswith("CONTENT_CALIBRATION_")] + [
    "ContentCalibrationContract",
    "ContentCalibrationUnit",
    "deterministic_calibration_run_id",
    "load_content_calibration_contract",
]
