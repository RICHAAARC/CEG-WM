...
def _validate_lf_balanced_blocks_selection_choices(config: Mapping[str, Any]) -> None:
    """Validate the one predeclared balanced-block fallback selection."""
    ...
    if provenance != {
        "lf_a3_exact": "09d84108d5b1ed22dd08a9793b4470268dcf76e9",
        "lf_a3_outcome": "SCIENTIFIC_NEGATIVE_core_A2_B8_shell_A6_B8_no_winner",
        "lf_v2_exact": "2c06993766a34730e3ecdcfc5e01efcf70795ce2",
        "lf_v2_outcome": "SCIENTIFIC_NEGATIVE_blocknorm_A6_B8_no_winner",
        "fallback_activation": "predeclared_only_after_primary_blocknorm_scientific_negative",
        "role": "pilot_negative_provenance_only_not_balanced_blocks_evidence",
    }:
        raise ValueError("balanced-block fallback must preserve both frozen LF negatives")
    ...
    if runtime != {
        ...
        raise ValueError("balanced-block runtime identity differs from the frozen production path")
    ...
    if keying != {
        ...
        "wrong_key_count": 16,
        "wrong_key_derivation_domain": "stage-a/external-wrong-key/v1",
        "carrier_domain_public_inputs": "carrier_method_id_C_H_W_channel_block_canonical_bounds_purpose_only",
        "primary_null": True,
        "payload_bits": 0,
    }:
        raise ValueError("balanced-block key/domain semantics differ from the frozen plan")
        ...
        "radial_blocks": [
            {
                ...
                "radius": [0.14, 0.165],
                "upper_bound": "exclusive",
                "canonical_bound_token": "0.14<=r<0.165",
            },
            {
                "block_index": 1,
                "radius": [0.165, 0.19],
                "upper_bound": "exclusive",
                "canonical_bound_token": "0.165<=r<0.19",
            },
            {
                "block_index": 2,
                "radius": [0.19, 0.215],
                "upper_bound": "exclusive",
                "canonical_bound_token": "0.19<=r<0.215",
            },
            {
                "block_index": 3,
                "radius": [0.215, 0.24],
                "upper_bound": "inclusive",
                "canonical_bound_token": "0.215<=r<=0.24",
            },
        ],
        "active_coordinate_rule": "lexicographic_rfft_coordinates_excluding_kx_zero_and_even_width_nyquist",
        "pair_order_rule": "sort_by_32_byte_prg_token_then_ky_then_kx",
        "pair_sign_rule": "pair_receives_plus_sign_minus_sign_and_final_unpaired_coordinate_is_zero",
        "construction_dtype": "float64",
        "component_weight": "one_over_sqrt_four_times_channels",
        "detector_statistic": "per_channel_block_independent_centered_cosine_then_equal_weight_deterministic_median",
        "invalid_block_rule": "fewer_than_two_active_zero_norm_invalid_roundtrip_support_or_nonfinite_fails_closed",
    }:
        raise ValueError("balanced-block carrier or detector identity differs from preregistration")

    budget = _require_mapping(config, "budget")
    if budget != {
        "total_relative_l2": 0.012,
        "measurement": "actual_dtype_final_minus_actual_dtype_base",
        "injection": "one_combined_carrier_with_existing_actual_dtype_backoff",
        "precast_component_energy": "equal_within_explicit_numerical_tolerance_not_post_float16_claim",
        "quality_evidence": {
            "actual_dtype_relative_l2": "enforced",
            "rgb_psnr": "reported_candidate_image_vs_primary_null",
            "lpips_alex": "not_evaluated",
        ...
        raise ValueError("balanced-block carrier must preserve the actual-dtype 0.012 budget")

    candidate_id = "lf_shell_balanced_blocks_v2_blocknorm_median_v1"
    if config.get("record_arms_in_exact_unit_order") != [
        candidate_id,
        f"primary_null__{candidate_id}",
    ]:
        raise ValueError("balanced-block selection requires the exact paired record-arm order")
    controls = _require_mapping(config, "controls")
    if controls != {
        "correct_key": "registered_detection_key",
        "wrong_key": "16_external_domain_separated_keys",
        ...
        raise ValueError("balanced-block controls differ from the frozen plan")
    ...
    if rule != {
        ...
        raise ValueError("balanced-block scale-free selection gates differ from preregistration")

    flow = _require_mapping(config, "execution_flow")
    if flow != {
        "selection_manifest": "lf_balanced_blocks_selection.jsonl",
        "selection_units": 8,
        "fixed_records": 16,
        "unit_transaction_record_count": 2,
        "untouched_confirmation_manifest_reserved": "lf_balanced_blocks_untouched_confirmation.jsonl",
        "untouched_confirmation_units_reserved": 8,
        "failure_units_remain_in_denominator": True,
        "replacement_units_allowed": False,
        "outcome_requires_complete_rc0": True,
        ...
        raise ValueError("balanced-block execution flow must preserve the 8-unit/16-record denominator")


def load_lf_balanced_blocks_selection_protocol(
    config_path: str | Path,
    selection_path: str | Path,
) -> StageAProtocol:
    """Load only balanced-block selection; its confirmation roster stays untouched."""

    config_file = Path(config_path)
    with config_file.open("r", encoding="utf-8") as handle:
        config = json.load(handle)
    if not isinstance(config, dict) or config.get("protocol_version") != 1:
        raise ValueError("unsupported balanced-block selection protocol version")
    protocol_id = _require_nonempty_text(config, "protocol_id")
    if protocol_id != "cegwm-stage-a-lf-balanced-blocks-selection-v1":
        raise ValueError("unexpected balanced-block selection protocol identity")
    _validate_lf_balanced_blocks_selection_choices(config)
    selection = _load_units(Path(selection_path), "lf_balanced_blocks_selection")
    expected_ids = [f"lfbb-selection-{index:04d}" for index in range(1, 9)]
    if len(selection) != 8 or [unit.unit_id for unit in selection] != expected_ids:
        raise ValueError("balanced-block selection manifest differs from the fresh fixed roster")
    digest_payload = {
        "config": config,
        "candidate_selection": [asdict(unit) for unit in selection],
    }
...
def _validate_lf_balanced_blocks_confirmation_choices(config: Mapping[str, Any]) -> None:
    """Validate only the frozen balanced-block untouched confirmation."""

    _validate_detection_access(config)
    if config.get("execution_scope_id") != "lf_balanced_blocks_untouched_confirmation_v1":
        raise ValueError("balanced-block confirmation execution scope identity differs")
    if config.get("scope_exclusions") != [
        ...
        "fixed_fpr_formal_evaluation",
        "paper_results",
    ]:
        raise ValueError("balanced-block confirmation scope exclusions differ")
    provenance = _require_mapping(config, "selection_provenance")
    if provenance != {
        ...
        raise ValueError("balanced-block confirmation selection provenance differs")
    ...
    if runtime != {
        ...
        raise ValueError("balanced-block confirmation runtime identity differs")
    keying = _require_mapping(config, "keying")
    if keying != {
        "task": "zero_bit_keyed_attribution",
        "normalization": "NFC_UTF8_for_text_exact_bytes_for_binary",
        ...
        raise ValueError("balanced-block confirmation key/domain semantics differ")
    candidate = _require_mapping(config, "candidate")
    if candidate != {
        "carrier_method_id": "lf_shell_balanced_blocks_v2",
        "evaluated_candidate_id": "lf_shell_balanced_blocks_v2_blocknorm_median_v1",
        ...
        raise ValueError("balanced-block confirmation carrier or detector identity differs")
    budget = _require_mapping(config, "budget")
    if budget != {
        "total_relative_l2": 0.012,
        "measurement": "actual_dtype_final_minus_actual_dtype_base",
        ...
        raise ValueError("balanced-block confirmation must preserve actual-dtype 0.012 budget")
    candidate_id = "lf_shell_balanced_blocks_v2_blocknorm_median_v1"
    if config.get("record_arms_in_exact_unit_order") != [
        candidate_id,
        f"primary_null__{candidate_id}",
    ]:
        raise ValueError("balanced-block confirmation requires the exact paired record arms")
    controls = _require_mapping(config, "controls")
    if controls != {
        "correct_key": "registered_detection_key",
        "wrong_key": "16_external_domain_separated_keys",
        ...
        raise ValueError("balanced-block confirmation controls differ")
    rule = _require_mapping(config, "confirmation_rule")
    if rule != {
        "fixed_units": 8,
        "registered_top_rank_among_17_min_units": 7,
        "paired_lf_registered_gt_primary_null_registered_min_units": 7,
        "strict_comparison_ties_fail": True,
        "pass_outcome": "confirmation_pass_candidate_for_agent5_adjudication",
        "failure_outcome": "SCIENTIFIC_NEGATIVE_AND_STOP",
        "absolute_margin_role": "reported_effect_size_only_no_pass_threshold",
        "primary_null_role": "reported_separately_no_pass_cutoff",
        "formal_threshold_status": "not_introduced_at_stage_a",
        ...
        raise ValueError("balanced-block confirmation scale-free gates differ")
    flow = _require_mapping(config, "execution_flow")
    if flow != {
        "untouched_confirmation_manifest": "lf_balanced_blocks_untouched_confirmation.jsonl",
        "untouched_confirmation_units": 8,
        "fixed_records": 16,
        "unit_transaction_record_count": 2,
        "failure_units_remain_in_denominator": True,
        "replacement_units_allowed": False,
        ...
        raise ValueError("balanced-block confirmation must preserve the 8-unit/16-record denominator")


def load_lf_balanced_blocks_confirmation_protocol(
    config_path: str | Path,
    confirmation_path: str | Path,
) -> StageAProtocol:
    """Load only the reserved balanced-block untouched confirmation roster."""

    with Path(config_path).open("r", encoding="utf-8") as handle:
        config = json.load(handle)
    if not isinstance(config, dict) or config.get("protocol_version") != 1:
        raise ValueError("unsupported balanced-block confirmation protocol version")
    protocol_id = _require_nonempty_text(config, "protocol_id")
    if protocol_id != "cegwm-stage-a-lf-balanced-blocks-confirmation-v1":
        raise ValueError("unexpected balanced-block confirmation protocol identity")
    _validate_lf_balanced_blocks_confirmation_choices(config)
    confirmation = _load_units(
        Path(confirmation_path),
        "lf_balanced_blocks_untouched_confirmation",
    )
    expected_ids = [f"lfbb-confirmation-{index:04d}" for index in range(1, 9)]
    if len(confirmation) != 8 or [unit.unit_id for unit in confirmation] != expected_ids:
        raise ValueError("balanced-block confirmation manifest differs from the frozen roster")
    digest_payload = {
        "config": config,
        "untouched_confirmation": [asdict(unit) for unit in confirmation],
    }
