"""Clean-null whitening fit for content-whitening."""
...
CONTENT_WHITENING_METHOD_ID = "content_v4_clean_null_whitened_lf_adaptive_hf_v1"
CONTENT_WHITENING_EVALUATED_CANDIDATE_ID = (
    "content_v4_clean_null_whitened_lf_adaptive_hf_semantic_gate_v1"
)
CONTENT_WHITENING_LF_SCORER_ID = "content_v4_whitened_lf_dct_matched_cosine_v1"
...
@dataclass(frozen=True, slots=True)
class FrozenContentWhiteningLFPublicAssets:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(carrier, FrozenLFPublicAssets):
            raise TypeError("content-whitening LF carrier assets must be frozen public assets")
        if (
            ...
            raise ValueError("content-whitening requires the current balanced LF carrier")
...
def load_frozen_content_whitening_asset(repo_root: str | Path) -> WhiteningAsset:
    ...
    if hashlib.sha256(asset_bytes).hexdigest() != ASSET_SHA256:
        raise ValueError("content-whitening whitening asset SHA differs")
    if hashlib.sha256(sidecar_bytes).hexdigest() != ASSET_SIDECAR_SHA256:
        raise ValueError("content-whitening whitening sidecar SHA differs")
    expected_sidecar = f"{ASSET_SHA256}  {Path(ASSET_REPO_PATH).name}\n".encode("ascii")
    if sidecar_bytes != expected_sidecar:
        raise ValueError("content-whitening whitening sidecar content differs")
    asset = load_whitening_asset(asset_path)
    if asset.payload["producer_exact"] != ASSET_PRODUCER_EXACT:
        raise ValueError("content-whitening whitening producer exact differs")
    return asset
...
def decode_whitening_weights(asset: WhiteningAsset) -> torch.Tensor:
    ...
    if not isinstance(asset, WhiteningAsset):
        raise TypeError("content-whitening whitening asset type differs")
    payload = _validate_asset_payload(asset.payload)
    if stable_json_bytes(payload) != asset.json_bytes:
        raise ValueError("content-whitening whitening asset bytes differ from its payload")
    ...
    if not weights.is_contiguous() or not bool(torch.isfinite(weights).all()):
        raise ValueError("decoded content-whitening whitening weights are invalid")
    return weights


def _detection_observation(image: Any, assets: FrozenContentWhiteningLFPublicAssets) -> torch.Tensor:
    ...
    if tuple(observation.shape) != OBSERVATION_SHAPE:
        raise ValueError("content-whitening final-image observation shape differs")
    if tuple(observation.stride()) != OBSERVATION_STRIDE:
        raise ValueError("content-whitening final-image observation order differs")
    if not bool(torch.isfinite(observation).all()):
        raise ValueError("content-whitening final-image observation must be finite")
    return observation


def score_content_whitened_lf_image(
    image: Any,
    detection_key: str | bytes | bytearray | memoryview,
    frozen_public_assets: FrozenContentWhiteningLFPublicAssets,
) -> float:
    """Blind matched cosine after public VAE, detrend, DCT, and fixed W."""

    if not isinstance(frozen_public_assets, FrozenContentWhiteningLFPublicAssets):
        raise TypeError("content-whitening LF score requires frozen public assets")
