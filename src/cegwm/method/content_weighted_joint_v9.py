"""Content V9 paired-null calibration and fixed weighted joint statistic."""
...
def fit_weighted_joint_calibration(pairs: Iterable[LFHFScorePair]) -> CalibrationFit:
    ...
    if len(received) != CONTENT_V9_PAIRED_NULL_SCORE_COUNT:
        raise ValueError("Content V9 calibration requires exactly 1056 ordered LF/HF pairs")
    count = len(received)
    ...
    if not math.isfinite(m2_lf) or not math.isfinite(m2_hf) or m2_lf <= 0.0 or m2_hf <= 0.0:
        raise ValueError("Content V9 calibration variances must be finite and positive")
    ...
    if not all(math.isfinite(value) for value in (mu_lf, sigma_lf, mu_hf, sigma_hf, rho)):
        raise ValueError("Content V9 calibration statistics must be finite")
    if sigma_lf <= 0.0 or sigma_hf <= 0.0 or not -1.0 <= rho <= 1.0:
        raise ValueError("Content V9 calibration sigma/rho domain differs")
...
def _joint_denominator(rho: Any) -> float:
    ...
    if not math.isfinite(value) or value <= 0.0:
        raise ValueError("Content V9 joint denominator must be finite and positive")
    return value
...
def weighted_joint_score(lf_score: Any, hf_score: Any, asset: WeightedJointAsset) -> float:
    ...
    if not isinstance(asset, WeightedJointAsset):
        raise TypeError("Content V9 weighted score requires a frozen calibration asset")
    ...
    if not math.isfinite(value):
        raise ValueError("Content V9 weighted joint score must be finite")
    return value
...
def weighted_gate_evidence(
    ...
    if len(wrong) != 16:
        raise ValueError("Content V9 weighted Gate A requires exactly 16 wrong keys")
...
def _validate_asset_payload(value: Any) -> Mapping[str, Any]:
    if not isinstance(value, dict) or tuple(value) != tuple(sorted(value)):
        raise ValueError("Content V9 calibration asset fields must use stable order")
    required = {
    ...
    expected = {
        ...
        "calibration_manifest_sha256": CONTENT_V9_CALIBRATION_MANIFEST_SHA256,
        ...
        "schema_version": CONTENT_V9_CALIBRATION_ASSET_SCHEMA_ID,
    ...
    if not -1.0 <= fit.rho <= 1.0:
        raise ValueError("Content V9 calibration rho differs")
...
def build_calibration_asset(
    *, producer_exact: str, protocol_digest: str, public_key_digest: str,
    ...
    if not isinstance(fit, CalibrationFit):
        raise TypeError("Content V9 asset requires a CalibrationFit")
    payload = {
        "asset_role_id": CONTENT_V9_CALIBRATION_ASSET_ROLE_ID,
        "base_exact": CONTENT_V9_BASE_EXACT,
        "calibration_manifest_sha256": CONTENT_V9_CALIBRATION_MANIFEST_SHA256,
...
def load_calibration_asset(path: str | Path, sidecar_path: str | Path) -> WeightedJointAsset:
    ...
    if Path(sidecar_path).read_bytes() != f"{digest}  {asset_path.name}\n".encode("ascii"):
        raise ValueError("Content V9 calibration asset sidecar binding differs")
    try:
        payload = json.loads(raw)
    except (UnicodeDecodeError, json.JSONDecodeError) as error:
        raise ValueError("Content V9 calibration asset must be UTF-8 JSON") from error
    validated = _validate_asset_payload(payload)
    if stable_json_bytes(validated) != raw:
        raise ValueError("Content V9 calibration asset must use stable JSON")
    return WeightedJointAsset(validated, raw)
