"""content-unweighted: content-allocated LF amplitude with adaptive spatial HF.

Content analysis and allocation are the substantive frozen content-adaptive implementation.
content-unweighted changes only the LF embedding direction: the standard balanced-block
...
from cegwm.method import content_adaptive as _content_adaptive
...
LF_CONTENT_UNWEIGHTED_EMBEDDING_TRANSFORM_ID = (
    "lf_unweighted_balanced_blocks_content_allocated_amplitude_v3"
)
CONTENT_UNWEIGHTED_METHOD_ID = "content_v3_unweighted_lf_adaptive_hf_v1"
CONTENT_UNWEIGHTED_EVALUATED_CANDIDATE_ID = (
    "content_v3_unweighted_lf_adaptive_hf_semantic_gate_v1"
)
_CONTENT_UNWEIGHTED_COUNTERFACTUAL_SIGNAL_FIELDS = (
    "semantic_importance",
    "texture_complexity",
...
def _content_unweighted_production_control_vector(
    allocation: ContentAllocation,
) -> torch.Tensor:
    """Return only allocation controls consumed by content-unweighted production."""
...
def allocate_content(signals: ContentSignals) -> ContentAllocation:
    """Allocate with the content-adaptive formulas and measure production controls."""

    allocation = _content_adaptive.allocate_content(signals)
    observed = _content_unweighted_production_control_vector(allocation)
    effects: list[float] = []
    for field in _CONTENT_UNWEIGHTED_COUNTERFACTUAL_SIGNAL_FIELDS:
        neutral_value = 0.0 if field in {
            "semantic_importance",
            "texture_complexity",
        } else 0.5
        counterfactual = _content_adaptive.allocate_content(
            replace(signals, **{field: (neutral_value,) * TILE_COUNT})
        )
        effect = float(torch.linalg.vector_norm(
            observed - _content_unweighted_production_control_vector(counterfactual)
        ).item())
        if not math.isfinite(effect) or effect < 0.0:
            raise RuntimeError(f"{field} content-unweighted counterfactual effect is invalid")
...
def _normalized_amplitude(carrier: torch.Tensor, amplitude: torch.Tensor) -> torch.Tensor:
    ...
    if not bool(torch.isfinite(norm)) or float(norm.item()) == 0.0:
        raise RuntimeError("content-unweighted carrier direction is invalid")
...
def _content_unweighted_branch_deltas(
    ...
    if not bool(torch.isfinite(base_l2)) or float(base_l2.item()) == 0.0:
        raise ValueError("content-unweighted requires a finite nonzero latent")
...
def embed_content_unweighted(
    ...
    """Embed content-unweighted LF and HF simultaneously under one actual-dtype budget."""
    ...
    if (
        ...
        raise TypeError("content-unweighted embedding requires floating NCHW callback latents")
    if not bool(torch.isfinite(latents).all()):
        raise ValueError("content-unweighted callback latents must be finite")
    if not isinstance(allocation, ContentAllocation):
        raise TypeError("content-unweighted embedding requires a real ContentAllocation")
    ...
    if any(
        ...
        raise RuntimeError("both content-unweighted branches must be nonzero")
    ...
    if measurement.perturbation_l2 == 0.0 or measurement.relative_l2 > COMBINED_RELATIVE_L2:
        raise RuntimeError("content-unweighted actual-dtype embedding is zero or over budget")
    if lf_actual.perturbation_l2 == 0.0 or hf_actual.perturbation_l2 == 0.0:
        raise RuntimeError("both actual-dtype content-unweighted branches must remain nonzero")
