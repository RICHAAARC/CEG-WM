...
def build_threshold_asset(
    ...
    *,
    ...
    """Freeze ``tau=max(z_i)`` only for a complete fixed 256-row run."""
    ...
    tau = max(row.z for row in checked)
    payload = dict(
        sorted(
            {
    ...
    asset = BlindThresholdAsset(payload, raw)
    if replay_empirical_false_positives(checked, asset) != 0:
        raise RuntimeError("threshold replay must produce exactly 0/256 empirical false positives")
    return asset
...
def replay_empirical_false_positives(
    ...
    if not isinstance(asset, BlindThresholdAsset):
        raise TypeError("threshold replay requires a BlindThresholdAsset")
...
def _validate_threshold_payload(payload: Any, raw: Any) -> None:
    ...
    required = {
        "decision_rule", "denominator", "disjoint_from", "fpr_experiment",
        "producer_exact", "replay_false_positives", "roster_digest", "rows_digest",
        "schema_version", "statistic_id", "tau_blind_be_hex", "value_dtype",
    ...
    for name in ("roster_digest", "rows_digest"):
...
__all__ = [
    "BLIND_DEV_DENOMINATOR", "BLIND_DEV_DISJOINT_FROM", "BLIND_STATISTIC_ID",
    "BLIND_THRESHOLD_SCHEMA_ID", "BLIND_WRONG_KEY_COUNT", "BlindCalibrationRoster",
    "BlindCalibrationRow", "BlindCalibrationUnit", "BlindStatistic", "BlindThresholdAsset",
    "build_test_only_threshold_asset", "build_threshold_asset", "decode_binary64",
