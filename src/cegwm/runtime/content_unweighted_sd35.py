"""Real SD3.5 step-18 adapter for the content-unweighted method candidate."""
...
class ContentUnweightedEmbedAssets:
    ...
    content_method_id: str = field(default=CONTENT_UNWEIGHTED_METHOD_ID, init=False)
    ...
    lf_embedding_transform_id: str = field(
        default=LF_CONTENT_UNWEIGHTED_EMBEDDING_TRANSFORM_ID, init=False
    )
    evaluated_candidate_id: str = field(
        default=CONTENT_UNWEIGHTED_EVALUATED_CANDIDATE_ID, init=False
    )

    def __post_init__(self) -> None:
        if self.dino_asset_id != DINO_ASSET_ID:
            raise ValueError("content-unweighted embed asset must be facebook/dinov2-small")
        _validate_dino_assets(self.dino_model, self.dino_processor)
        if not isinstance(self.hf_public_assets, FrozenHFPublicAssets):
            raise TypeError("content-unweighted HF assets must be FrozenHFPublicAssets")
        if not isinstance(self.lf_public_assets, FrozenLFPublicAssets):
            raise TypeError("content-unweighted LF assets must be FrozenLFPublicAssets")
        ...
        if (
            ...
            raise ValueError("content-unweighted HF frozen carrier or detector identity differs")
        if (
            ...
            != LF_BALANCED_BLOCKS_EVALUATED_CANDIDATE_ID
            or self.content_method_id != CONTENT_UNWEIGHTED_METHOD_ID
            or self.lf_embedding_transform_id
            != LF_CONTENT_UNWEIGHTED_EMBEDDING_TRANSFORM_ID
            or self.evaluated_candidate_id
            != CONTENT_UNWEIGHTED_EVALUATED_CANDIDATE_ID
        ):
            raise ValueError("content-unweighted LF carrier, detector, or method identity differs")
        if hf.vae is not lf.vae or hf.image_processor is not lf.image_processor:
            raise ValueError(
                "content-unweighted HF and LF detectors must share public observation assets"
            )


@dataclass(frozen=True, slots=True)
class ContentUnweightedRunOutput:
...
class ContentUnweightedInjectionCallback:
    """Analyze content and embed both content-unweighted branches once at step 18."""
    ...
    def __init__(
        ...
        assets: ContentUnweightedEmbedAssets,
    ) -> None:
        if not isinstance(assets, ContentUnweightedEmbedAssets):
            raise TypeError("content-unweighted callback requires ContentUnweightedEmbedAssets")
    ...
    def __call__(
        ...
        if self._measurement is not None:
            raise RuntimeError("content-unweighted callback attempted step 18 more than once")
        latents = callback_kwargs.get("latents")
        if not isinstance(latents, torch.Tensor):
            raise TypeError("diffusers callback state must contain torch latents")
        if pipeline is None:
            raise RuntimeError("content-unweighted analysis requires the real callback pipeline")
        ...
        embedded, measurement = embed_content_unweighted(
            latents,
            self._detection_key,
...
def run_sd35_content_unweighted(
    pipeline: Any,
    prompt: str,
    detection_key: str | bytes | bytearray | memoryview,
    assets: ContentUnweightedEmbedAssets,
    *,
...
) -> ContentUnweightedRunOutput:
    """Run one real content-unweighted joint embedding and return public outputs only."""
    ...
    callback = ContentUnweightedInjectionCallback(detection_key, assets)
    result = pipeline(
        prompt=prompt,
    ...
    if callback.measurement is None:
        raise RuntimeError("pipeline completed without content-unweighted step-18 embedding")
    return ContentUnweightedRunOutput(
...
__all__ = [
    ...
    "ContentUnweightedInjectionCallback",
    "ContentUnweightedRunOutput",
    "RUNTIME_ASSET_VALIDATION_CONTRACT_ID",
    "load_dino_content_assets",
