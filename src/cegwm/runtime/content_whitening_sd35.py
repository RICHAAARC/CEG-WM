"""Thin current-stack clean RGB observation path for the content-whitening W fit."""
...
def materialize_clean_fit_observation(
    ...
    if tuple(observation.shape) != OBSERVATION_SHAPE:
        raise ValueError("content-whitening fit observation must have shape 1x16x64x64")
    ...
    if (
        ...
        raise ValueError("content-whitening fit observation must be exact C-order storage")
    if not bool(torch.isfinite(materialized).all()):
        raise ValueError("content-whitening fit observation must be finite")
    return materialized
...
def run_clean_fit_observation(
    ...
    *,
    ...
    if not isinstance(entry, FitEntry):
        raise TypeError("content-whitening fit runtime requires a validated FitEntry")
    if not isinstance(generator, torch.Generator):
        raise TypeError("content-whitening fit runtime requires an explicit torch Generator")
