...
from cegwm.formal_experiment_v2 import (
...
from cegwm.runtime.paper_detection_v2 import score_current_rgb as _score_current_rgb, detect_paper_v2
...
CONFIG_PATH = REPO_ROOT / "configs/paper_experiment/formal_experiment_v2.json"
METHOD_ID = "cegwm_continuous_single_h_paper_v2"
SYNCSEAL_RESIDUAL_MULTIPLIER = 0.75
PREFLIGHT_PROMPT = "A neutral geometric still life for an engineering preflight."
PREFLIGHT_SEED = 2028000000
...
def verify_expected_exact(expected_exact: str) -> None:
    """Code revision is recorded; main checkout is allowed."""
    return None
...
def _detect_payload(runtime: Mapping[str, Any], image: Image.Image, tau: float, *, no_geometry: bool = False) -> dict[str, Any]:
    ...
    record = detect_paper_v2(image, runtime["key"], runtime["assets"], tau)
...
def _build_runtime(runtime_root: Path) -> dict[str, Any]:
    ...
    # The existing factory downloads create-only; preflight and formal may share a runtime parent.
    checkpoint_root = Path(tempfile.mkdtemp(prefix="syncseal-", dir=runtime_root))
    pipeline, assets = build_production_runtime(
        REPO_ROOT, config, hf_token=token, runtime_root=checkpoint_root
    )
...
def run_worker(*, job_id: str, expected_exact: str, drive_root: Path, runtime_root: Path) -> int:
    ...
    if final_path.exists():
        ...
        if final.get("method_id") != METHOD_ID:
            raise RuntimeError("existing main final belongs to a different method")
...
def run_engineering_canary(
    *, job_id: str, expected_exact: str, drive_root: Path, runtime_root: Path,
    ...
    if final_path.exists():
        ...
        if value.get("science_denominator") != 0:
