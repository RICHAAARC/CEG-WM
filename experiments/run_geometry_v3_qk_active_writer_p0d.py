"""Bounded Geometry-V3 P0D single-configuration writer diagnostic.
...
P0D_PROTOCOL_ID = "geometry-v3-keyed-qk-active-writer-p0d-v1"
P0D_CONFIG = P0WriterConfig(4, 0.0025)
P0D_STATUS_STOPPED = "P0D_STOPPED"
P0D_STATUS_COMPLETE = "P0D_DIAGNOSTIC_COMPLETE"
P0D_SCIENCE_DENOMINATOR = 0
P0D_DRIVE_ROOT = "/content/drive/MyDrive/CEG-WM/Geometry-V3/P0D/"
P0D_ARTIFACT_MAX_BYTES = 32 * 1024
...
class P0DDiagnosticResult(NamedTuple):
    status: str
    failure_point: str
...
class DiagnosticWriterSession(ActiveQKWriterSession):
    ...
    def __init__(self, transformer: Any, counts: dict[str, int], anchor: Any) -> None:
        super().__init__(transformer, P0D_CONFIG, anchor)
...
def _generator_for(pipeline: Any) -> torch.Generator:
    ...
    try:
        ...
        raise RuntimeError("P0D transformer device is unavailable") from error
    ...
    *,
    ...
    try:
        ...
        if not isinstance(images, (list, tuple)) or len(images) != 1:
            raise RuntimeError("P0D generation did not return one RGB")
        image = require_ordinary_rgb_image(images[0])
        if image.size != (P0_IMAGE_SIZE, P0_IMAGE_SIZE):
            raise ValueError("P0D final RGB dimensions differ")
        counts["final_rgb_validation_count"] = 1
        return P0DDiagnosticResult(
            P0D_STATUS_COMPLETE, "none", None, dict(counts)
        )
...
def public_plan() -> dict[str, Any]:
    return {
        "protocol": P0D_PROTOCOL_ID,
        "model_id": P0_MODEL_ID,
        "config_id": P0D_CONFIG.config_id,
        "block_index": 4,
        "feature_kinds": ["q", "k"],
...
def package_p0d_artifacts(
    output_directory: Path,
    *,
    exact: str,
    result: P0DDiagnosticResult,
) -> dict[str, Any]:
    if output_directory.exists():
        raise FileExistsError("P0D output directory already exists")
    if result.status not in {P0D_STATUS_STOPPED, P0D_STATUS_COMPLETE}:
        raise ValueError("P0D status differs")
    if result.failure_point not in PUBLIC_FAILURE_POINTS:
        raise ValueError("P0D failure point differs")
    if tuple(result.counters) != COUNTER_NAMES:
        raise ValueError("P0D counter roster differs")
    if any(
        isinstance(value, bool) or not isinstance(value, int) or value < 0
        for value in result.counters.values()
    ):
        raise ValueError("P0D counters must be nonnegative integers")
    output_directory.mkdir(parents=True, exist_ok=False)
    run_id = f"geometry-v3-qk-p0d-{exact[:12]}"
    ...
    receipt = {
        ...
        "candidate_selection": False,
        "science_denominator": P0D_SCIENCE_DENOMINATOR,
    }
    terminal = {
        "run_id": run_id,
        "status": result.status,
    ...
    if sum(len(data) for data in payloads.values()) >= P0D_ARTIFACT_MAX_BYTES:
        raise RuntimeError("P0D artifact exceeds its bound")
    for name, data in payloads.items():
        with (output_directory / name).open("xb") as stream:
            stream.write(data)
    return {
        "run_id": run_id,
        "p0d_status": result.status,
        "artifact_status": "complete",
        "failure_point": result.failure_point,
        "error_class": result.error_class,
        "counters": result.counters,
...
def execute_plan(
    ...
    *,
    ...
    if set(plan) != {"expected_exact", "execution_exact", "output_directory"}:
        raise ValueError("P0D plan fields differ")
    ...
    if not isinstance(expected, str) or expected != execution or len(expected) != 40:
        raise ValueError("P0D execution identity differs")
    output = plan["output_directory"]
    if not isinstance(output, str) or not output.startswith(P0D_DRIVE_ROOT):
        raise ValueError("P0D output must use its create-only Drive namespace")
    if not geometry_key.strip() or not hf_token.strip():
        raise ValueError("P0D runtime credentials are required")
    result = run_p0d(
        geometry_key=geometry_key,
        hf_token=hf_token,
        preloader=preloader,
    )
    return package_p0d_artifacts(Path(output), exact=execution, result=result)


def _read_plan(path: Path) -> dict[str, Any]:
    size = path.stat().st_size
    if size <= 0 or size > MAX_PLAN_BYTES:
        raise ValueError("P0D plan size differs")
    raw = path.read_bytes()
    if len(raw) != size:
        raise ValueError("P0D plan read is incomplete")
    value = json.loads(raw)
    if not isinstance(value, dict) or len(_json_bytes(value)) > MAX_PLAN_BYTES:
        raise ValueError("P0D canonical plan differs")
    return value
...
def _git_exact(expected: str) -> str:
    ...
    if exact != expected or status:
        raise RuntimeError("P0D checkout identity or cleanliness differs")
    return exact
...
def _emit(fd: int, value: dict[str, Any]) -> None:
    ...
    if len(payload) > MAX_CONTROL_BYTES:
        raise RuntimeError("P0D control receipt exceeds its bound")
    os.write(fd, payload)
    ...
    *,
    ...
    try:
        ...
        if not isinstance(expected, str):
            raise ValueError("P0D expected exact is missing")
        _git_exact(expected)
        stage = "execution"
        try:
