"""Real clean runner for content-adaptive dual-branch evaluation."""
...
def _validate_content_record(
    ...
    *,
    ...
    if tuple(record) != RECORD_FIELDS:
        raise ValueError("content-adaptive record fields or order differ from the frozen contract")
    if record["record_contract_id"] != selected.record_contract_id:
        raise ValueError("content-adaptive record contract identity differs")
    for name in (
        "run_id", "unit_id", "source_cluster_id", "arm", "condition",
        "code_revision", "config_digest", "key_public_digest", "status",
    ):
        if not isinstance(record[name], str) or not record[name].strip():
            raise ValueError(f"content-adaptive record {name} must be nonempty text")
    if record["arm"] not in selected.arms or record["condition"] != "clean":
        raise ValueError("content-adaptive record arm or condition differs")
    if re.fullmatch(r"[0-9a-f]{40}", record["code_revision"]) is None:
        raise ValueError("content-adaptive record revision must be an exact lowercase commit")
    if any(
        re.fullmatch(r"[0-9a-f]{64}", record[name]) is None
        for name in ("config_digest", "key_public_digest")
    ):
        raise ValueError("content-adaptive record digest identity differs")
    status = record["status"]
    failure_reason = record["failure_reason"]
    scores = record["scores"]
    metrics = record["metrics"]
    if not isinstance(scores, dict) or not isinstance(metrics, dict):
        raise TypeError("content-adaptive record scores and metrics must be plain JSON objects")
...
def _content_record(
    *,
    ...
    _validate_content_record(record, variant=selected)
    return record
...
def _load_protocol(repo_root: Path) -> ContentChainProtocol:
    ...
    return load_content_adaptive_protocol(
...
def _candidate_aggregate_metrics(
    ...
    *,
    ...
    if measurement.probe_evaluation_count != 64:
        raise ValueError("content-adaptive candidate measurement must contain exactly 64 probe evaluations")
    result = {
        "unit_id": unit_id,
