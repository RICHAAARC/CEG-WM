"""User-run producer for the content ISS detector-domain ISS g/m asset."""
...
FIT_RECEIPT_PREFIX = "CEGWM_CONTENT_ISS_FIT_RECEIPT"
...
def _git_exact(repo_root: Path, expected_exact: str) -> str:
    ...
    if exact != expected_exact or status:
        raise RuntimeError("content ISS ISS producer checkout identity differs")
    return exact
...
def _require_create_only(asset_path: Path, sidecar_path: Path) -> None:
    if asset_path.exists() or sidecar_path.exists():
        raise FileExistsError("create-only content ISS ISS asset destination exists")
...
def _load_pipeline_and_assets(token: str) -> tuple[Any, ContentISSDevelopmentAssets]:
    if not torch.cuda.is_available():
        raise RuntimeError("cuda_required_for_real_content_ISS_fit")
    pipeline, v4_assets = v4_runner._load_pipeline_and_assets(
        "stabilityai/stable-diffusion-3.5-medium", token
    )
    return pipeline, ContentISSDevelopmentAssets(
...
def execute(args: argparse.Namespace) -> int:
    ...
    if not root_key_text:
        token = ""
        raise RuntimeError("CEG_WM_ROOT_KEY_is_required_for_content_ISS_fit")
    development_key = derive_development_key(root_key_text)
    root_key_text = ""
    if not token.strip():
        development_key = b""
        raise RuntimeError("HF_TOKEN_is_required_for_content_ISS_fit")
    try:
    ...
    if len(measurements) != 32:
        raise RuntimeError("content ISS ISS fit did not complete exactly 32 units")
