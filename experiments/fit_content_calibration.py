"""User-run producer for the content paired-null calibration asset."""
...
from cegwm.protocol.content_calibration import (
    ...
    CONTENT_CALIBRATION_CALIBRATION_RECEIPT_ID,
...
TOKEN_ENV = "HF_TOKEN"
ASSET_FILENAME = f"{CONTENT_CALIBRATION_CALIBRATION_ASSET_ROLE_ID}.json"
RECEIPT_PREFIX = "CEGWM_CONTENT_CALIBRATION_CALIBRATION_RECEIPT"
...
def _git_exact(repo_root: Path, expected_exact: str) -> str:
    ...
    if exact != expected_exact or status:
        raise RuntimeError("content calibration checkout identity differs")
    return exact
...
def _require_create_only(asset_path: Path, sidecar_path: Path) -> None:
    if asset_path.exists() or sidecar_path.exists():
        raise FileExistsError("create-only content calibration destination exists")
...
def derive_calibration_key(root_key: str | bytes | bytearray | memoryview) -> bytes:
    return prg_bytes(normalize_detection_key(root_key), CONTENT_CALIBRATION_CALIBRATION_KEY_DOMAIN, 32)


def _load_pipeline_and_assets(token: str) -> tuple[Any, ContentCalibrationAssets]:
    if not torch.cuda.is_available():
        raise RuntimeError("cuda_required_for_real_content_calibration")
    pipeline, assets = iss_runner._load_pipeline_and_assets(
...
def _receipt(
    *, asset_sha256: str, exact: str, protocol_digest: str,
    ...
    payload = {
        ...
        "receipt_contract_id": CONTENT_CALIBRATION_CALIBRATION_RECEIPT_ID,
        "run_id": run_id,
...
def execute(args: argparse.Namespace) -> int:
    ...
    contract = load_content_calibration_contract(repo_root)

    root_key_text = os.environ.pop(KEY_ENV, "")
    token = os.environ.pop(TOKEN_ENV, "")
    if not root_key_text:
        token = ""
        raise RuntimeError("CEG_WM_ROOT_KEY_is_required_for_content_calibration")
    ...
    if not token.strip():
        calibration_key = b""
        raise RuntimeError("HF_TOKEN_is_required_for_content_calibration")
    try:
    ...
    for unit in contract.calibration:
        pairs.extend(run_content_calibration_unit(pipeline, unit, calibration_key, assets))
    if len(pairs) != CONTENT_CALIBRATION_PAIRED_NULL_SCORE_COUNT:
        raise RuntimeError("content calibration did not produce exactly 1056 pairs")
