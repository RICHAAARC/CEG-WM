"""One-unit, non-roster operational canary for the real Content V6 runtime."""
...
from experiments.run_content_v6_clean import (
    _load_pipeline_and_assets,
    _run_pair,
)
from cegwm.method.content_adaptive_v3 import BRANCH_SHARE_SUM_ABSOLUTE_TOLERANCE
...
def _assert_non_roster_identity(repo_root: Path) -> None:
    protocol = load_content_v6_clean_protocol(repo_root)
    if (
        protocol.protocol_id != CONTENT_V6_PROTOCOL_ID
        or protocol.protocol_digest != CONTENT_V6_PROTOCOL_DIGEST
    ):
        raise RuntimeError("canary protocol identity differs from Content V6")
...
def _registered_scores(image: Any, detection_key: bytes, assets: Any) -> dict[str, float]:
    ...
    lf_score = _finite_scalar(
        score_content_v4_lf_image(ordinary, detection_key, assets.lf_public_assets),
        "LF score",
    )
...
def execute(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        run_id = f"{CONTENT_V6_RUN_PREFIX}-{CONTENT_V6_PROTOCOL_DIGEST[:12]}-{key_digest[:12]}"
