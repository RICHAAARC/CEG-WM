"""Real method/runtime runner for the eight-cluster LF transport diagnostic."""
...
from main import LfDetectionObservation, derive_wrong_key_material
from runtime import ContentWriteVaeResult, Sd35RuntimeAdapter


class LfTransmissionRunnerError(RuntimeError):
    """The real LF transport diagnostic violated its frozen contract."""
...
def _tensor_values(tensor: torch.Tensor) -> tuple[float, ...]:
    if (
        ...
        raise LfTransmissionRunnerError("LF signal tensor is invalid")
    return tuple(
        float(item)
...
class LfTransmissionDiagnosticRunner:
    """Calls the registered LF carrier, embedder, runtime and blind detector."""
    ...
    def __init__(
        ...
        *,
        ...
        if type(adapter) is not CegWmExperimentAdapter:
            raise LfTransmissionRunnerError("method adapter exact type is required")
        if type(runtime_adapter) is not Sd35RuntimeAdapter:
            raise LfTransmissionRunnerError("runtime adapter exact type is required")
        if type(method_code_revision) is not str or len(method_code_revision) != 40:
            raise LfTransmissionRunnerError("method revision is invalid")
    ...
    def _analysis_identity(
        self, entry: LfTransmissionManifestEntry
    ) -> AnalysisUnitIdentity:
        return derive_lf_transmission_analysis_identity(
            entry,
            self.manifest,
            ...
            bindings.append(
                create_frozen_development_unit_binding(
                    ...
                    scientific_question_id=(
                        "lf_signal_survival_across_generation_boundaries"
                    ),
                    development_case_id="paired_clean_lf_transport_observation",
    ...
    def _score_position(
        ...
        if position_id == "rgb_vae_reencoded":
            raise LfTransmissionRunnerError(
        ...
        registered_carrier = self.adapter.build_lf_carrier(
            self.registered_root_key, shape
        ).result
        wrong_carrier = self.adapter.build_lf_carrier(wrong_key, shape).result
        ...
        return create_lf_signal_position_observation(
    ...
    def _score_reencoded_public_image_position(
        ...
        candidate_tensor: torch.Tensor,
        clean_tensor: torch.Tensor,
    ) -> tuple[LfSignalPositionObservation, dict[str, object]]:
        candidate_observation = _observation(candidate_tensor)
        clean_observation = _observation(clean_tensor)
        wrong_key = derive_wrong_key_material(self.root_key_public_digest, 0)
        registered = self.adapter.detect_lf(
            candidate_observation, self.registered_root_key
        ).result
        wrong = self.adapter.detect_lf(candidate_observation, wrong_key).result
        primary_null = self.adapter.detect_lf(
        ...
        if not (
            ...
            raise LfTransmissionRunnerError("LF detector configuration drifted")
        observation = create_lf_signal_position_observation(
            position_id="rgb_vae_reencoded",
            statistic_role="formal_lf_detector_operation",
            registered_score=registered.lf_score,
            wrong_key_score=wrong.lf_score,
    ...
    def _execute_paired_runtime(
        ...
        *,
        ...
        if type(result) is not ContentWriteVaeResult:
            raise LfTransmissionRunnerError("runtime result exact type is required")
        elapsed = float(monotonic() - started)
        if not isfinite(elapsed) or elapsed <= 0.0:
            raise LfTransmissionRunnerError("paired runtime elapsed time was not measured")
    ...
    def execute_scientific_cluster(
        ...
        *,
        ...
        if type(cluster_ordinal) is not int or not 0 <= cluster_ordinal < 8:
            raise LfTransmissionRunnerError("cluster ordinal is outside frozen manifest")
        ...
        materialization_scalars = (
            materialization.realized_total_l2,
            materialization.realized_relative_l2,
            materialization_result.content_relative_l2_nominal,
            materialization_result.content_relative_l2_limit,
            materialization_result.realized_total_l2,
            materialization_result.realized_relative_l2,
            materialization_result.budget_utilization,
            materialization_result.materialization_scale,
        )
        if (
            ...
            or any(not isfinite(float(value)) for value in materialization_scalars)
        ):
            raise LfTransmissionRunnerError(
                "LF content materialization did not pass finite budget and integrity gates"
            )
        ...
        elapsed = float(monotonic() - started)
        if not isfinite(elapsed) or elapsed < 0.0:
            raise LfTransmissionRunnerError("cluster elapsed time is invalid")
        ...
        operation_payload = {
            ...
            "content_materialization_result": {
                ...
                "attempt_count": materialization_result.attempt_count,
                ...
                "embedding_result_identity": (
                    materialization_result.embedding_result.embedding_result_identity
                ),
            },
            ...
            "formal_lf_detection_results": formal_detector_results,
        }
        final = positions[-1]
        metric_payload = {
            ...
            "candidate_config_digest": self.candidate_config_digest,
            "paired_ablation_identity": "paired_clean_lf_same_generation",
            ...
            "threshold_role": "not_fitted_lf_transmission_diagnostic",
        ...
        record_payload = {
            ...
            "scientific_question_id": "lf_signal_survival_across_generation_boundaries",
            "development_case_id": "paired_clean_lf_transport_observation",
            "candidate_identity": self.protocol.candidate_identity,
            "candidate_config_digest": self.candidate_config_digest,
            "paired_ablation_identity": "paired_clean_lf_same_generation",
    ...
    def create_failed_scientific_record(
        ...
        *,
        ...
        if not 0 <= cluster_ordinal < 8:
            raise LfTransmissionRunnerError("failed cluster is outside frozen manifest")
        if (
            ...
            raise LfTransmissionRunnerError("failed cluster facts are invalid")
        entry = self.manifest.entries[cluster_ordinal]
        identity = self._analysis_identity(entry)
        operation_payload = {
            "failure_stage": "lf_transmission_runtime_operation",
        ...
        record_payload = {
            ...
            "failure_reason": "lf_transmission_runtime_operation_failed",
            "retry_parent_intent_digest": retry_parent_intent_digest,
    ...
    def replay_directional_decision(
        ...
        if len(evidence) != 8:
            raise LfTransmissionRunnerError("verified LF cluster coverage is incomplete")
        indexes = tuple(record.unit_index for record, _marker in evidence)
        if indexes != tuple(range(8)) or len(set(indexes)) != 8:
            raise LfTransmissionRunnerError("verified LF unit indexes drifted")
        source_ids: set[str] = set()
        final_observations: list[LfSignalPositionObservation] = []
        failure_count = 0
        for record, marker in evidence:
            ...
            if (
                ...
                or record.responsibility_id != "lf_detector"
                ...
                raise LfTransmissionRunnerError("verified LF evidence binding drifted")
            identity = AnalysisUnitIdentity(**record.analysis_unit_identity)
            if identity.source_cluster_id in source_ids:
                raise LfTransmissionRunnerError("verified LF source cluster duplicated")
            ...
            if identity != self._analysis_identity(expected_entry):
                raise LfTransmissionRunnerError("verified LF source cluster is foreign")
            ...
            if type(raw_positions) not in {tuple, list} or len(raw_positions) != 4:
                raise LfTransmissionRunnerError("verified LF positions are incomplete")
            try:
                positions = tuple(
                    LfSignalPositionObservation(**item) for item in raw_positions
                )
            except (TypeError, ValueError) as exc:
                raise LfTransmissionRunnerError(
                    "verified LF position schema drifted"
                ) from exc
            for position in positions:
                position.validate()
            if tuple(item.position_id for item in positions) != self.protocol.signal_positions:
                raise LfTransmissionRunnerError("verified LF position order drifted")
            final_observations.append(positions[-1])
        return evaluate_lf_transmission_direction(
            final_observations,
            budget_integrity_nonfinite_failure_count=failure_count,
        )


__all__ = ["LfTransmissionDiagnosticRunner", "LfTransmissionRunnerError"]
