"""Formal fixed-order evaluation for the accepted content-chain asset."""
...
from experiments import content_iss_engine as iss_runner
from cegwm.method.content_adaptive import COUNTERFACTUAL_EFFECT_FIELDS
...
ARMS = (
    CONTENT_CHAIN_EVALUATED_CANDIDATE_ID,
    f"primary_null__{CONTENT_CHAIN_EVALUATED_CANDIDATE_ID}",
)
...
COMPLETE_EXECUTION = "complete_for_content_chain_evaluation"
INCOMPLETE_EXECUTION = "incomplete_content_chain_execution"


def _ordered_units(contract: ContentChainContract) -> tuple[ContentChainUnit, ...]:
    return (
        *contract.reference_roster,
...
def _identity(
    contract: ContentChainContract, *, exact: str, key_digest: str
) -> dict[str, Any]:
    ...
    return {
        "run_id": run_id,
        "exact": exact,
        "execution_scope_id": CONTENT_CHAIN_EXECUTION_SCOPE_ID,
        "protocol_id": contract.config["protocol_id"],
        "protocol_digest": contract.protocol_digest,
        "public_key_digest": key_digest,
        "calibration_asset_sha256": CONTENT_CHAIN_CALIBRATION_ASSET_SHA256,
        ...
        "record_contract_id": CONTENT_CHAIN_RECORD_CONTRACT_ID,
        ...
        "artifact_contract_id": CONTENT_CHAIN_ARTIFACT_CONTRACT_ID,
    }


def _new_state(identity: dict[str, Any], now: float) -> dict[str, Any]:
    return {
        "state_schema_id": CONTENT_CHAIN_STATE_SCHEMA_ID,
        "identity": identity,
...
def _validate_record(record: Any, *, unit: ContentChainUnit, arm_index: int,
                     identity: Mapping[str, Any]) -> None:
    if not isinstance(record, dict) or tuple(record) != RECORD_FIELDS:
        raise ValueError("content chain record fields or order differ")
    if (
        ...
        or record["record_contract_id"] != CONTENT_CHAIN_RECORD_CONTRACT_ID
    ):
        raise ValueError("content chain record identity differs")
    if record["status"] == "operational_failure":
        if record["failure_reason"] not in engine._PUBLIC_OPERATIONAL_ERROR_CLASSES:
            raise ValueError("content chain failure class differs")
        if record["scores"] or record["metrics"]:
            raise ValueError("content chain failure record must be sanitized")
        return
    if record["status"] != "success" or record["failure_reason"] is not None:
        raise ValueError("content chain record status differs")
    scores = record["scores"]
    if not isinstance(scores, dict) or tuple(scores) != SCORE_FIELDS:
        raise ValueError("content chain score fields or order differ")
    for name, value in scores.items():
        scalar = _finite(value, name)
        if not name.startswith("weighted_joint__") and not -1.0 <= scalar <= 1.0:
            raise ValueError("content chain branch score escaped [-1,1]")
    ...
    if not isinstance(metrics, dict) or set(metrics) != set(expected_metrics):
        raise ValueError("content chain metric fields differ")
    for name in expected_metrics:
        _finite(metrics[name], name)
    if metrics["paired_rgb_psnr_db"] < 0.0:
        raise ValueError("content chain PSNR must be nonnegative")
    if arm_index == 0:
        if not 0.0 <= metrics["combined_relative_l2"] <= 0.012:
            raise ValueError("content chain combined budget differs")
        if not all(
            0.0 < metrics[name] <= 0.012
            for name in ("lf_effective_relative_l2", "hf_effective_relative_l2")
        ):
            raise ValueError("content chain effective branch budget differs")
        if not (
            ...
            raise ValueError("content chain branch shares differ")
        effects = [metrics[name] for name in COUNTERFACTUAL_EFFECT_FIELDS]
        if any(value < 0.0 for value in effects):
            raise ValueError("content chain counterfactual effect differs")
        if metrics["minimum_counterfactual_effect"] != min(effects):
            raise ValueError("content chain minimum effect differs")
        if metrics["probe_evaluation_count"] != 64.0:
            raise ValueError("content chain probe count differs")


def _validate_state(
    state: Any, identity: dict[str, Any], contract: ContentChainContract
) -> dict[str, Any]:
    if not isinstance(state, dict) or tuple(state) != STATE_FIELDS:
        raise ValueError("content chain state fields or order differ")
    if state["state_schema_id"] != CONTENT_CHAIN_STATE_SCHEMA_ID:
        raise ValueError("content chain state schema differs")
    if not isinstance(state["identity"], dict) or tuple(state["identity"]) != IDENTITY_FIELDS:
        raise ValueError("content chain identity fields or order differ")
    if not engine._same_json_bytes(state["identity"], identity):
        raise ValueError("content chain state identity differs")
    sequence = state["checkpoint_sequence"]
    committed = state["committed_unit_count"]
    if not isinstance(sequence, int) or isinstance(sequence, bool) or sequence < 0:
        raise ValueError("content chain checkpoint sequence differs")
    if not isinstance(committed, int) or isinstance(committed, bool) or not 0 <= committed <= 80:
        raise ValueError("content chain committed count differs")
    anchor = _finite(state["checkpoint_time_anchor_unix_seconds"], "checkpoint time anchor")
    if anchor < 0.0:
        raise ValueError("content chain checkpoint anchor differs")
    records = state["records"]
    if not isinstance(records, list) or len(records) != committed * 2:
        raise ValueError("content chain state is not a whole-unit prefix")
    ...
    for unit_index in range(committed):
        ...
        if statuses not in (("success", "success"), ("operational_failure", "operational_failure")):
            raise ValueError("content chain transaction status is incomplete")
        if statuses[0] == "operational_failure" and (
            transaction[0]["failure_reason"] != transaction[1]["failure_reason"]
        ):
            raise ValueError("content chain transaction failure classes differ")
    return state
...
def _read_sink_state(archive: Path, sidecar: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise ValueError("content chain checkpoint state must be an object")
    return value


def _load_sink_checkpoint(
    sink_run_root: Path, identity: dict[str, Any], contract: ContentChainContract
) -> dict[str, Any] | None:
    ...
    if set(archives) != set(sidecars):
        raise ValueError("content chain checkpoint pairs are incomplete")
    if not archives:
        return None
    sequences = sorted(archives)
    if sequences != list(range(sequences[-1] + 1)):
        raise ValueError("content chain checkpoint sequence is not contiguous")
    ...
    for sequence in sequences:
        ...
        if state["checkpoint_sequence"] != sequence + 1:
            raise ValueError("content chain checkpoint name and metadata differ")
        if previous is not None and (
            ...
            raise ValueError("content chain checkpoint history diverges")
        previous = state
    return previous
...
def _resolve_state(
    *, local_state_path: Path, sink_run_root: Path, identity: dict[str, Any],
    contract: ContentChainContract, now: float,
) -> dict[str, Any]:
    ...
    if sink is None:
        if local["checkpoint_sequence"] != 0:
            raise ValueError("content chain local checkpoint history is missing from sink")
        return local
    if sink["checkpoint_sequence"] > local["checkpoint_sequence"]:
        if not engine._same_json_bytes({"records": sink["records"]}, {"records": local["records"]}):
            raise ValueError("content chain checkpoint reconciliation differs")
        engine._write_local_state(local_state_path, sink)
        return sink
    if sink["checkpoint_sequence"] < local["checkpoint_sequence"]:
        raise ValueError("content chain sink checkpoint history rolls back local state")
    if len(sink["records"]) > len(local["records"]):
        raise ValueError("content chain sink contains uncommitted history")
    if not engine._same_json_bytes(
        ...
        raise ValueError("content chain local and sink histories diverge")
    return local
...
def _flat_scores(scores: Mapping[str, Mapping[str, float]]) -> dict[str, float]:
    if tuple(scores) != SCORE_BRANCHES or any(tuple(scores[name]) != SCORE_LABELS for name in SCORE_BRANCHES):
        raise ValueError("content chain blind score fields differ")
    return {
...
def _record(
    *, identity: Mapping[str, Any], unit: ContentChainUnit, arm_index: int,
...
def _failure_transaction(
    unit: ContentChainUnit, identity: Mapping[str, Any], error: Exception
) -> list[dict[str, Any]]:
...
def _unit_transaction(
    *, pipeline: Any, unit: ContentChainUnit, key: bytes,
    wrong_keys: tuple[bytes, ...], assets: ContentCalibrationAssets,
    contract: ContentChainContract, identity: Mapping[str, Any],
) -> list[dict[str, Any]]:
    output = run_content_chain_unit(
        pipeline, unit, key, wrong_keys, assets, contract.calibration_asset
    )
    if not isinstance(output, ContentChainOutput):
        raise TypeError("content chain unit requires its real runtime output")
    metrics = engine._candidate_aggregate_metrics(
        unit.unit_id,
        output.measurement,
        engine._psnr(output.image, output.primary_null),
        share_sum_absolute_tolerance=contract.runtime_protocol.config["aggregate_measurement"][
    ...
    if tuple(candidate_scores) != ("lf", "hf", "weighted_joint") or tuple(null_scores) != (
        ...
        raise ValueError("content chain runtime score branches differ")
    return [
        _record(
...
def _section_result(
    *, section_id: str, units: Sequence[ContentChainUnit], records: list[dict[str, Any]],
...
def _result(
    state: Mapping[str, Any], identity: Mapping[str, Any], contract: ContentChainContract,
...
def _receipt(identity: Mapping[str, Any], committed: int) -> dict[str, Any]:
    return {
        ...
        "artifact_contract_id": CONTENT_CHAIN_ARTIFACT_CONTRACT_ID,
        "receipt_contract_id": CONTENT_CHAIN_TERMINAL_RECEIPT_ID,
        "run_id": identity["run_id"],
        "exact": identity["exact"],
        "execution_scope_id": identity["execution_scope_id"],
        "protocol_id": identity["protocol_id"],
...
def _load_pipeline_and_assets(model_id: str, token: str) -> tuple[Any, ContentCalibrationAssets]:
    pipeline, assets = iss_runner._load_pipeline_and_assets(model_id, token)
    return pipeline, ContentCalibrationAssets(assets.evaluation_assets)
