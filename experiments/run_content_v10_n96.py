...
def execute(args: argparse.Namespace) -> int:
 secret=os.environ.pop(c1.KEY_ENV,""); token=os.environ.pop(c1.TOKEN_ENV,""); local_run=None; c1_published=False
 try:
  ...
  for ordinal,line in enumerate((root/"configs/content_chain/content_texture_n96_evaluation_v1.jsonl").read_text(encoding="utf-8").splitlines()):
   unit=json.loads(line); from cegwm.runtime.content_v10_texture_neutral_sd35 import derive_v10_calibration_wrong_keys, run_content_v10_paired_evaluation
   ...
   for name,scores,asset,fn in (("v9",v9,v9_asset,weighted_joint_score),("v10",v10,v10_asset,weighted_joint_v10)):
    row[name]["lf_b"]=scores[0][0]-plain[0][0]; row[name]["hf_b"]=scores[0][1]-plain[0][1]; row[name]["joint_a"]=fn(*scores[0],asset)-max(fn(*x,asset) for x in scores[1:]); row[name]["joint_b"]=fn(*scores[0],asset)-fn(*plain[0],asset)
   rows.append(row)
  if len(rows)!=96: raise ValueError("fixed N96 denominator differs")
  result=sink/run_id/RESULT; result_payload=c1.stable_json_bytes({"exact":exact,"manifest_digest":TEXTURE_N96_MANIFEST_DIGEST,"c1_asset_sha256":digest,"v9_asset_sha256":"63c17e8200a92383b061541fc234dfef36e4b7356954c160ce5f048f820cde96","fixed_units":96,"rows":rows,"claim_ceiling":n96_contract.config["claim_ceiling"]}); result_sha=_publish_result(result,result_payload)
  ...
  print(PREFIX+" "+c1.stable_json_bytes({"status":"incomplete","completeness":"incomplete","scientific_status":"not_evaluable","fixed_units":96,"result_path":None,"sidecar_path":None,"result_sha256":None}).decode("ascii"),flush=True); return 2
 finally:
