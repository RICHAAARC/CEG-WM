...
All four units must pass all fixed requirements at a given amplitude. Content compatibility additionally requires the content-only and content+geometry six frozen LF/HF/weighted-joint diagnostic booleans to be complete, identical, and positive. Any one fixed amplitude with a complete 4/4 pass emits `CARRIER_WINDOW_CANDIDATE` and records `passing_amplitudes`; otherwise it fails closed. This remains a carrier-window candidate only: `science_denominator=0`, no formal FPR, robustness, or scientific-success claim.
