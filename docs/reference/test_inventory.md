...
| `tests/unit/test_runtime_qk_observation.py` | `unit`, `quick` | yes | CPU fake backend 覆盖普通图像 Batch-3 Q/K hook、公开噪声、冻结 forward identity、精确 module callable 构造锚定与执行前后复验，以及 callable 预置/执行中替换的 clean FAILED 语义；不加载模型、不访问网络或 GPU。 |
...
| `tests/unit/test_c1_hf_reference_protocol.py` | `unit` | yes | C1-P HF-only spec、pinned PartiPrompts TSV SHA、明文 prompt/digest/category/challenge/source-row roster、两个 compact manifests 的 4096-cluster 确定性物化摘要、prompt 互斥并集与逐 `(category, challenge)` 分层平衡、完整 method/runtime/adapter 权威绑定、fit/confirmation 隔离、独立 tau artifact 前置、统计可证伪门、actual-dtype hard budget、工作量与 shard 边界、metric identities/split bindings 和 protocol 不含正式 metric 实现；仅 CPU schema/constraint，不是 C1-E 或科学结果。 |
| `tests/unit/test_c1_hf_reference_metrics.py` | `unit` | yes | C1-M 七项真实 metric 实现：exact materialized-manifest 4096-unit threshold/confirmation、binary64 nextafter、shared exact CP upper/lower、registered/wrong paired attribution、流式单 pair HWC RGB8 relative-L2/MSE、4096 轻量 case-result Student-t aggregate、actual-dtype 失败分母及 score/quality/actual 三表交叉绑定；正式入口逐 pair 重放 raw RGB8 artifact descriptors、重算完整 fit-null tau，并拒绝可自重哈希的伪 quality 数值与错误 threshold。覆盖完整 C1-P authority byte tamper、identity/digest/duplicate/missing/non-finite 与 result forgery fail closed。仅 CPU fixture，不写 records、不执行 C1-E/GPU/promotion，也不构成科学结果。 |
...
| `tests/unit/test_experiment_execution_delivery.py` | `unit`, `quick`, `constraint` | yes | A3b 确定性 exact-revision package builder、allowlist/exclusion、dirty/revision/archive/bootstrap/unsafe-path fail-closed、正式 content/geometry callable 完整声明与漂移，以及薄且无输出的 package-external-bootstrap Notebook；不运行模型、GPU 或科学实验。 |
| `tests/unit/test_experiment_execution_package_integration.py` | `integration` | no | repository-side 临时 exact-revision 构包、包外 SHA/manifest/allowlist/逐文件验证、安全解包、包内导入、A3a runner record/replay、结果包及 extracted package-contained tests 闭环；本文件不进入 archive。 |
...
| `tests/smoke/test_packaged_experiment_execution.py` | `smoke` | no | archive 显式 allowlist 内的真实 A3a runner/writer/replay 单 case CPU/synthetic 接线，只使用包内 module/files，不读取 Git、不初始化模型 backend。 |
