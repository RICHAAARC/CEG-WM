...
| `tests/unit/test_internal_scientific_validation_protocol.py` | `unit` | yes | 内部 unit/case/source-cluster identity、八 split 无泄漏、伪造 grant 与最终 held-out fail-closed、四类执行状态、retry/stop 集合状态机、协议版本绑定、13 职责验证矩阵、同 detector/config/threshold 和内容证据阳性/阴性约束；仅 CPU schema/constraint，不宣称科学有效。 |
