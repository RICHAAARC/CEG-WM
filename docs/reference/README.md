...
| [migration/](migration/README.md) | 历史 source registry、构建前 13 组件 migration plan，以及未来逐组件 manifest/readiness 的分离边界。 |
...
只保存构建前 `not_started` 计划；实际迁移后的 provenance 必须进入 future
per-component manifest；实现完成门禁只进入 future method readiness。四者不得互相
替代。
