...
| [cpu_validation_environment.md](cpu_validation_environment.md) | 使用登记 Conda 环境运行完整 CPU 三门，并限定 `.venv` 的轻量治理用途。 |
...
拆包内容以 `docs/reference/extraction_profiles.md` 为准；外层执行工具不属于研究文档或交付内容。当前没有单独的数据准备、GPU 调度或正式实验执行指南。
