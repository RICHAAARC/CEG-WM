...
- `colab/development_exploration.ipynb` 是当前唯一允许 **Run all** 的入口。它绑定
  revision `b66cb04ebb41f0d5473c498ad5769b467ff26d7e` 和全新 run
  `ceg_wm_thirteen_module_mechanism_screening_operational_validation`，以 Drive 为跨会话
  persistent root，
  并只调用该 revision 中的统一 development server。本次入口固定传入
  `--maximum-wiring-clusters 2` 与 `--stop-before-scientific-units`。首次最多运行 2 个
  preflight 加 2 个 wiring operational units，共 4 operational、0 scientific；后续
  Run all 每次最多恢复 2 个 wiring，累计 10 个 operational 后停止，重复运行不会进入
  unit 10 或创建 scientific 产物。完整筛查必须使用以后单独审核的入口。
...
operational-validation run 不读取、不续跑、不迁移、不改写、不删除上述任一历史 namespace。
