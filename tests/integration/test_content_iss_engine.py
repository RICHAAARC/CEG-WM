...
# Functional coverage for the content ISS engine.
...
from experiments import content_iss_engine as runner
from cegwm.protocol.content_iss import CONTENT_ISS_PROTOCOL_DIGEST

_ROOT = Path(__file__).resolve().parents[2]


@pytest.mark.integration
def test_iss_runner_binds_final_protocol_and_paired_variant() -> None:
    protocol = runner._load_protocol(_ROOT)
    variant = runner.CONTENT_ISS_RUNNER_VARIANT
    ...
    assert variant.lf_scorer is runner.score_content_whitened_lf_image


@pytest.mark.integration
def test_generic_engine_uses_iss_pair_primary_null_without_third_generation(
    ...
    variant = replace(runner.CONTENT_ISS_RUNNER_VARIANT, run_pair=paired)
    monkeypatch.setattr(
        engine,
        "run_sd35_plain",
        lambda *args, **kwargs: pytest.fail("content ISS engine must not create a third generation"),
    )
