...
from experiments import fit_content_calibration as runner
from cegwm.method.content_weighted_joint import LFHFScorePair, load_calibration_asset
from cegwm.protocol.content_calibration import CONTENT_CALIBRATION_CALIBRATION_SPLIT, ContentCalibrationUnit


def _units() -> tuple[ContentCalibrationUnit, ...]:
    return tuple(
        ContentCalibrationUnit(
            f"content-v9-calibration-{index + 1:04d}", CONTENT_CALIBRATION_CALIBRATION_SPLIT,
...
def test_runner_calls_exactly_32_real_pairs_clears_secrets_and_publishes_create_only(
    ...
    calls: list[ContentCalibrationUnit] = []
    monkeypatch.setattr(runner, "_git_exact", lambda root, expected: exact)
    monkeypatch.setattr(
        runner, "load_content_calibration_contract",
    ...
    def fake_unit(pipeline: object, unit: ContentCalibrationUnit, key: bytes, assets: object):
        calls.append(unit)
        offset = len(calls) * 33
        return tuple(
            LFHFScorePair(
    ...
    monkeypatch.setattr(runner, "run_content_calibration_unit", fake_unit)
