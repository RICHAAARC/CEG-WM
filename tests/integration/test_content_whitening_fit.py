...
# Functional coverage for content-whitening fitting.
...
from experiments import fit_content_whitening as runner
from cegwm.method import content_whitening as whitening
from cegwm.runtime.content_whitening_sd35 import materialize_clean_fit_observation
...
def test_controlled_runner_exactly_32_clean_calls_and_public_create_only_asset(
    ...
    manifest = whitening.load_fit_manifest(_REPO_ROOT / whitening.FIT_MANIFEST_REPO_PATH)
    ...
    loaded_asset = whitening.load_whitening_asset(asset_path)
    ...
    assert receipt_output.count("CEGWM_CONTENT_WHITENING_RECEIPT ") == 1
...
def test_strict_final_rgb_observation_materialization_and_shape_failure() -> None:
    ...
    assert observation.shape == torch.Size(whitening.OBSERVATION_SHAPE)
    assert observation.dtype == torch.float32
    assert observation.device.type == "cpu"
    assert tuple(observation.stride()) == whitening.OBSERVATION_STRIDE
