...
from cegwm.protocol.content_chain_v3 import CONTENT_V3_PROTOCOL_DIGEST
...
def _identity():
    ...
    identity = engine._public_identity(
        ...
        run_id=run_id,
        variant=runner.CONTENT_V3_RUNNER_VARIANT,
    )
    ...
    for index, unit in enumerate(protocol.roster):
        ...
        records.extend((
            engine._content_v2_record(
                ...
                arm=runner.CONTENT_V3_RUNNER_VARIANT.arms[0],
                condition="clean",
            ...
            engine._content_v2_record(
                ...
                unit_id=unit.unit_id,
                source_cluster_id=unit.source_id,
                arm=runner.CONTENT_V3_RUNNER_VARIANT.arms[1],
                condition="clean",
                code_revision=identity["exact"],
                config_digest=identity["protocol_digest"],
                key_public_digest=identity["public_key_digest"],
...
def test_content_v3_runner_binds_distinct_deterministic_run_record_and_state_identity() -> None:
    ...
    state = engine._new_state(
        identity, 1.0, variant=runner.CONTENT_V3_RUNNER_VARIANT
    )
    ...
    v2 = load_content_adaptive_dual_branch_v2_clean_protocol(
        root / "content_adaptive_dual_branch_v2_clean_v1.json",
        root / "content_adaptive_dual_branch_v2_clean.jsonl",
    )
    v2_identity = engine._public_identity(
        v2,
        exact=_EXACT,
        key_digest=identity["public_key_digest"],
        run_id="content-adaptive-v2-e3fe3fd32ca2-805bc21e173a",
    )
    assert v2_identity["ordered_arms"] == list(engine.ARMS)
    assert v2_identity["record_contract_id"] == engine.RECORD_CONTRACT_ID
    assert v2_identity["execution_scope_id"] == engine.EXECUTION_SCOPE_ID
    assert v2_identity["public_key_digest"] == identity["public_key_digest"]


@pytest.mark.integration
def test_content_v3_runner_fixed_records_gates_strict_ties_and_formal_fpr_false() -> None:
    ...
    engine._validate_state(
        state, identity, protocol, variant=runner.CONTENT_V3_RUNNER_VARIANT
    )
    result = engine._derive_result(
        records, protocol, identity, variant=runner.CONTENT_V3_RUNNER_VARIANT
    )
...
@pytest.mark.integration
def test_content_v3_wrong_keys_blind_lf_hf_min_joint_and_detector_boundary(
...
@pytest.mark.integration
def test_content_v3_failure_transaction_stays_in_denominator_without_secret_or_retry() -> None:
    protocol, identity = _identity()
    records = [
        engine._content_v2_record(
            run_id=identity["run_id"],
        ...
        for arm in runner.CONTENT_V3_RUNNER_VARIANT.arms
    ]
    state = engine._new_state(
        identity, 1.0, variant=runner.CONTENT_V3_RUNNER_VARIANT
    )
