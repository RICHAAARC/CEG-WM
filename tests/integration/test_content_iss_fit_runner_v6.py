...
from experiments import run_content_v6_iss_fit as runner
from cegwm.method.content_iss_v6 import ISSDevelopmentMeasurement, load_iss_asset
from cegwm.protocol.content_chain_v6 import ContentV6Unit, V6_DEVELOPMENT_SPLIT


def _units() -> tuple[ContentV6Unit, ...]:
    return tuple(
        ContentV6Unit(
            f"content-v6-iss-dev-{index+1:04d}", V6_DEVELOPMENT_SPLIT,
...
def test_fit_runner_checks_destination_before_secret_or_pipeline(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    exact = "3" * 40
    monkeypatch.setattr(runner, "_git_exact", lambda root, expected: exact)
    monkeypatch.setattr(runner, "load_content_v6_data_contract", lambda root: SimpleNamespace(development=_units()))
