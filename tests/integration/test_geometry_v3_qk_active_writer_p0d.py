...
def _load_runner():
    ...
    spec = importlib.util.spec_from_file_location("geometry_v3_p0d_runner", path)
...
def test_real_p0d_writer_path_counts_one_fixed_complete_session() -> None:
    result = RUNNER.run_p0d(
        geometry_key="geometry-private-key-0001",
        hf_token="hf-private-token",
        preloader=_preloader_for(_FakePipeline()),
    )
    assert result.status == RUNNER.P0D_STATUS_COMPLETE
...
def test_p0d_stages_fail_closed_with_public_counters(
    preloader,
    expected_stage: str,
    expected_error: str,
    ...
    assert result.status == RUNNER.P0D_STATUS_STOPPED
...
def _invoke_main(
    ...
    monkeypatch.setattr(RUNNER, "P0D_DRIVE_ROOT", root.as_posix() + "/")
...
@pytest.mark.parametrize(
    ...
    [
        (_FakePipeline(), "complete", RUNNER.P0D_STATUS_COMPLETE, "none"),
        (
            RuntimeError("private model failure"),
            "pipeline-load-stopped",
            RUNNER.P0D_STATUS_STOPPED,
            "pipeline_load",
        ),
        (
...
def test_real_main_packages_bounded_public_diagnostic_artifacts(
    ...
    assert control["p0d_status"] == status
    ...
    assert len(payload) < RUNNER.P0D_ARTIFACT_MAX_BYTES
...
def test_runner_has_no_retry_fallback_or_stdout_stderr_control_path() -> None:
    ...
    assert "p0d_config = p0writerconfig(4, 0.0025)" in source
