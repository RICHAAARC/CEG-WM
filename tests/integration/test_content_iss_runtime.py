...
# Functional coverage for the content ISS runtime.
...
from cegwm.runtime import content_iss_sd35 as runtime


def _assets() -> runtime.ContentISSDevelopmentAssets:
    instance = object.__new__(runtime.ContentISSDevelopmentAssets)
    object.__setattr__(instance, "embed_assets", object())
    object.__setattr__(instance, "lf_public_assets", object())
    return instance


def _evaluation_assets() -> runtime.ContentISSEvaluationAssets:
    instance = object.__new__(runtime.ContentISSEvaluationAssets)
    object.__setattr__(instance, "embed_assets", object())
    object.__setattr__(instance, "lf_public_assets", object())
...
def test_real_pair_orchestration_is_plain_then_whitening_beta_one_with_reset_seed(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda pipeline, prompt, key, assets, **kwargs: calls.append(("whitening-beta-one", pipeline, prompt, key, assets, kwargs)) or SimpleNamespace(image="joint-image"),
    )
    ...
    unit = ContentISSUnit(
        "content-v6-iss-dev-0001", ISS_DEVELOPMENT_SPLIT,
        "content-v6-iss-dev-source-0001", "prompt", 2026082400, 512, 512,
    )
    assets = _assets()
    measurement = runtime.run_content_iss_development_pair(object(), unit, b"d"*32, assets)
    assert [call[0] for call in calls] == ["plain", "whitening-beta-one"]
...
def test_runtime_rejects_non_development_unit_before_generation(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    unit = ContentISSUnit("u", "wrong", "s", "p", 1, 512, 512)
    with pytest.raises(TypeError, match="validated dev unit"):
        runtime.run_content_iss_development_pair(object(), unit, b"d"*32, _assets())
...
def test_evaluation_pair_is_callback_free_pass1_then_same_seed_sole_pass2(
    ...
    output = runtime.run_content_iss_evaluation_pair(
        object(),
        "prompt",
        b"d" * 32,
        assets,
