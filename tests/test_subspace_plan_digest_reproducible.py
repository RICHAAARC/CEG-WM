"""
File purpose: Reproducibility test for subspace planner digest.
...
def test_subspace_plan_digest_reproducible() -> None:
    ...
    result_1 = planner.plan(cfg, mask_digest="mask_digest_repro", cfg_digest="cfg_digest_repro", inputs=inputs)
    result_2 = planner.plan(cfg, mask_digest="mask_digest_repro", cfg_digest="cfg_digest_repro", inputs=inputs)

    assert result_1.status == "ok"
    assert result_2.status == "ok"
    assert result_1.plan_digest == result_2.plan_digest
    assert result_1.basis_digest == result_2.basis_digest
