...
# Unit coverage for the content ISS protocol.
...
from cegwm.protocol.content_whitening import _DECISION_RULE as V4_DECISION_RULE

_ROOT = Path(__file__).resolve().parents[2]


@pytest.mark.unit
def test_iss_manifests_bind_exact_order_hashes_and_disjoint_data_roles() -> None:
    contract = load_content_iss_data_contract(_ROOT)
    assert ISS_PERSONAL_SPEC_SHA256 == (
    ...
    assert contract.development_manifest_sha256 == ISS_DEVELOPMENT_MANIFEST_SHA256
    assert contract.evaluation_manifest_sha256 == ISS_EVALUATION_MANIFEST_SHA256
    ...
    assert hashlib.sha256(dev_prompts).hexdigest() == ISS_DEVELOPMENT_PROMPT_LIST_SHA256
    assert hashlib.sha256(eval_prompts).hexdigest() == ISS_EVALUATION_PROMPT_LIST_SHA256


@pytest.mark.unit
def test_iss_manifest_loader_fails_closed_on_data_or_serialization_drift(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="manifest bytes differ"):
        load_content_iss_data_contract(tmp_path)


@pytest.mark.unit
def test_final_iss_protocol_binds_asset_pair_and_preserves_whitening_decisions() -> None:
    protocol = load_content_iss_protocol(_ROOT)
    assert protocol.protocol_id == CONTENT_ISS_PROTOCOL_ID
    assert protocol.protocol_digest == CONTENT_ISS_PROTOCOL_DIGEST
    ...
    assert tuple(config["execution_flow"]["record_arms_in_order"]) == CONTENT_ISS_ARMS
