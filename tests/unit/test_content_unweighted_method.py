...
# Unit coverage for the content-unweighted method.
...
from cegwm.method import content_adaptive as adaptive
from cegwm.method import content_unweighted as unweighted
...
def _signals(**updates: tuple[float, ...]) -> unweighted.ContentSignals:
    values = {
    ...
    return unweighted.ContentSignals(**values)
...
def _allocation(
    *,
    ...
    lf_share: float = 0.4,
) -> unweighted.ContentAllocation:
    return unweighted.ContentAllocation(
        lf_weights,
        hf_weights,
        lf_share,
        1.0 - lf_share,
...
@pytest.mark.unit
def test_content_unweighted_lf_is_collinear_unweighted_and_real_share_sets_amplitude() -> None:
    ...
    low_lf, _ = unweighted._content_unweighted_branch_deltas(
        base, b"registered-key-01", hf, lf, low
    )
    high_lf, _ = unweighted._content_unweighted_branch_deltas(
...
@pytest.mark.unit
def test_content_unweighted_real_signals_change_allocation_shares_and_branch_amplitudes() -> None:
    baseline = unweighted.allocate_content(_signals())
    controlled = (
        unweighted.allocate_content(_signals(
            semantic_importance=(0.05, 1.0) + (0.0,) * 14,
        )),
        unweighted.allocate_content(_signals(
            texture_complexity=(unweighted.RGB8_TEXTURE_COMPLEXITY_MAX,) * 16,
        )),
        unweighted.allocate_content(_signals(
    ...
    low_hf = unweighted.allocate_content(_signals(
    ...
    high_hf = unweighted.allocate_content(_signals(
        semantic_importance=(1.0,) * 16,
        texture_complexity=(unweighted.RGB8_TEXTURE_COMPLEXITY_MAX,) * 16,
    ...
    hf, lf = _assets()
    low_lf_delta, low_hf_delta = unweighted._content_unweighted_branch_deltas(
        base, b"registered-key-01", hf, lf, low_hf
    )
    high_lf_delta, high_hf_delta = unweighted._content_unweighted_branch_deltas(
...
@pytest.mark.unit
def test_content_unweighted_six_neutral_effects_use_real_allocation_and_only_production_controls() -> None:
    signals = _signals(
    ...
    allocation = unweighted.allocate_content(signals)
    v2_allocation = adaptive.allocate_content(signals)
    ...
    observed_unweighted = torch.tensor(
        (*allocation.hf_tile_weights, allocation.lf_branch_share, allocation.hf_branch_share),
        dtype=torch.float64,
    )
    expected_unweighted: list[float] = []
    for signal_field in fields(unweighted.ContentSignals):
        neutral = (
            0.0
            if signal_field.name in {"semantic_importance", "texture_complexity"}
            else 0.5
        )
        counterfactual = adaptive.allocate_content(
            replace(signals, **{signal_field.name: (neutral,) * unweighted.TILE_COUNT})
        )
        counterfactual_unweighted = torch.tensor(
            (
        ...
        expected_unweighted.append(float(torch.linalg.vector_norm(observed_unweighted - counterfactual_unweighted)))

    assert allocation.counterfactual_effects == pytest.approx(expected_unweighted, abs=1e-15)
    assert any(
    ...
    neutral = unweighted.ContentSignals(
        (0.0,) * unweighted.TILE_COUNT,
        (0.0,) * unweighted.TILE_COUNT,
        (0.5,) * unweighted.TILE_COUNT,
        (0.5,) * unweighted.TILE_COUNT,
        (0.5,) * unweighted.TILE_COUNT,
        (0.5,) * unweighted.TILE_COUNT,
    )
    assert unweighted.allocate_content(neutral).counterfactual_effects == (0.0,) * 6


@pytest.mark.unit
def test_content_unweighted_hf_nonuniform_weights_change_direction_and_adaptive_path_stays_spatial() -> None:
    base = torch.linspace(-1.0, 1.0, 4 * 64 * 64).reshape(1, 4, 64, 64)
    hf, lf = _assets()
    nonuniform = (0.5,) * 8 + (1.5,) * 8
    ...
    _, uniform_hf = unweighted._content_unweighted_branch_deltas(
        base, b"registered-key-01", hf, lf, uniform_allocation
    )
    _, weighted_hf = unweighted._content_unweighted_branch_deltas(
    ...
    v2_uniform = adaptive._lf_transformed_delta(carrier, (1.0,) * 16, amplitude)
    v2_weighted = adaptive._lf_transformed_delta(carrier, nonuniform, amplitude)
    assert not torch.allclose(v2_uniform, v2_weighted)


@pytest.mark.unit
@pytest.mark.parametrize("dtype", (torch.float16, torch.float32))
def test_content_unweighted_joint_actual_dtype_budget_branches_and_step18(dtype: torch.dtype) -> None:
    base = torch.linspace(-2.0, 2.0, 4 * 64 * 64).reshape(1, 4, 64, 64).to(dtype)
    hf, lf = _assets()
    allocation = unweighted.allocate_content(_signals())
    candidate, measurement = unweighted.embed_content_unweighted(
    ...
    assert 0.0 < actual <= unweighted.COMBINED_RELATIVE_L2
