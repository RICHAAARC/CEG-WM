...
NOTEBOOK = (
    ...
    / "notebooks"
    / "geometry_v3_qk_active_writer_p0d1_colab.ipynb"
)
RUNNER = (
    Path(__file__).resolve().parents[2]
    / "experiments"
...
RUNNER_EXACT = "dda330e804fd552407c461431646a4b46405475d"
...
def test_notebook_binds_exact_single_runner_control_and_drive_contract() -> None:
    ...
    assert f"P0D1_RUNNER_EXACT = '{RUNNER_EXACT}'" in code
    assert "git', 'checkout', '--detach', P0D1_RUNNER_EXACT" in code
    assert "resolved != P0D1_RUNNER_EXACT or dirty" in code
    assert "runner_path = checkout / RUNNER_RELATIVE_PATH" in code
    assert "'/content/drive/MyDrive/CEG-WM/Geometry-V3/P0D1'" in code
    ...
    assert "'status': control.get('p0d1_status')" in code
