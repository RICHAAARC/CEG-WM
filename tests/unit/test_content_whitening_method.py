...
# Unit coverage for the content-whitening method.
...
from cegwm.method import content_whitening as whitening
...
_MANIFEST = _REPO_ROOT / whitening.FIT_MANIFEST_REPO_PATH
...
def fitted() -> tuple[whitening.FitManifest, whitening.WhiteningFit, whitening.WhiteningAsset]:
    manifest = whitening.load_fit_manifest(_MANIFEST)
    fit = whitening.fit_whitening_operator(_observations())
    asset = whitening.build_whitening_asset(_PRODUCER_EXACT, fit.words_be_hex)
...
def test_complete_production_words_match_independent_oracle_and_fixed_golden(
    fitted: tuple[whitening.FitManifest, whitening.WhiteningFit, whitening.WhiteningAsset],
) -> None:
...
def test_manifest_count_and_uniqueness_fail_closed(
    ...
    with pytest.raises(ValueError, match=match):
        whitening.load_fit_manifest(path)
...
def test_compact_asset_round_trip(
    ...
    assert whitening.load_whitening_asset(path) == asset
...
def test_observation_contract_defects_fail_closed(defect: str) -> None:
    ...
    with pytest.raises((TypeError, ValueError), match=match):
        whitening.fit_whitening_operator(values)


@pytest.mark.unit
def test_degenerate_affine_observations_fail_closed() -> None:
    observation = torch.ones(whitening.OBSERVATION_SHAPE, dtype=torch.float32)
    with pytest.raises(ValueError, match="global whitening energy"):
        whitening.fit_whitening_operator(tuple(observation.clone() for _ in range(32)))
...
def test_whitening_word_contract_fails_closed(words: tuple[str, ...], match: str) -> None:
    with pytest.raises(ValueError, match=match):
        whitening.build_whitening_asset(_PRODUCER_EXACT, words)
...
def _detector_fixture() -> tuple[
    Image.Image,
    bytes,
    whitening.FrozenContentWhiteningLFPublicAssets,
    _DetectorProcessor,
    _DetectorVAE,
]:
    y, x = np.mgrid[:64, :64]
    ...
    asset = whitening.load_frozen_content_whitening_asset(_REPO_ROOT)
    return (
        image,
        b"content-v4-detector-golden-key",
        whitening.FrozenContentWhiteningLFPublicAssets(carrier_assets, asset),
        processor,
        vae,
    )
...
def _independent_detector_oracle(
    image: Image.Image,
    key: bytes,
    assets: whitening.FrozenContentWhiteningLFPublicAssets,
) -> float:
    pixels = np.asarray(image.resize((64, 64)), dtype=np.float32) / 255.0
...
def test_frozen_repository_asset_pair_exact_and_key_independent(tmp_path: Path) -> None:
    asset = whitening.load_frozen_content_whitening_asset(_REPO_ROOT)
    assert hashlib.sha256(asset.json_bytes).hexdigest() == whitening.ASSET_SHA256
    assert hashlib.sha256(
        (_REPO_ROOT / whitening.ASSET_SIDECAR_REPO_PATH).read_bytes()
    ).hexdigest() == whitening.ASSET_SIDECAR_SHA256
    weights = whitening.decode_whitening_weights(asset)
    ...
    assert asset.payload["schema_version"] == whitening.ASSET_SCHEMA_ID
    assert asset.payload["producer_exact"] == whitening.ASSET_PRODUCER_EXACT
    assert tuple(field.name for field in fields(whitening.FrozenContentWhiteningLFPublicAssets)) == (
        "carrier_assets", "whitening_asset"
    )
    assert tuple(inspect.signature(whitening.decode_whitening_weights).parameters) == ("asset",)

    root = tmp_path / "repo"
    destination = root / whitening.ASSET_REPO_PATH
    destination.parent.mkdir(parents=True)
    destination.write_bytes(asset.json_bytes)
    sidecar = root / whitening.ASSET_SIDECAR_REPO_PATH
    sidecar.write_bytes((_REPO_ROOT / whitening.ASSET_SIDECAR_REPO_PATH).read_bytes() + b"x")
    with pytest.raises(ValueError, match="sidecar SHA"):
        whitening.load_frozen_content_whitening_asset(root)


@pytest.mark.unit
def test_content_whitening_detector_matches_independent_oracle_golden_and_real_boundary() -> None:
    ...
    score = whitening.score_content_whitened_lf_image(image, key, assets)
    ...
    assert whitening.decode_whitening_weights(assets.whitening_asset).equal(
        whitening.decode_whitening_weights(assets.whitening_asset)
    )


@pytest.mark.unit
def test_content_whitening_detector_is_blind_and_fails_closed_on_degenerate_observation() -> None:
    image, key, assets, _, _ = _detector_fixture()
    assert tuple(inspect.signature(whitening.score_content_whitened_lf_image).parameters) == (
        "image", "detection_key", "frozen_public_assets"
    )
    with pytest.raises(TypeError):
        whitening.score_content_whitened_lf_image(image, key, assets, object())
    ...
    with pytest.raises(ValueError, match="denominator"):
        whitening.score_content_whitened_lf_image(
            image,
            key,
            whitening.FrozenContentWhiteningLFPublicAssets(zero_assets, assets.whitening_asset),
        )
