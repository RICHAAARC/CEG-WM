...
# Unit coverage for the content ISS method.
...
from cegwm.method import content_iss as iss
from cegwm.method.content_whitening import FrozenContentWhiteningLFPublicAssets, load_frozen_content_whitening_asset
...
def _assets() -> FrozenContentWhiteningLFPublicAssets:
    ...
    return FrozenContentWhiteningLFPublicAssets(
        carrier,
        load_frozen_content_whitening_asset(_ROOT),
    )


def _fit() -> iss.ISSFit:
    measurements = tuple(
        iss.ISSDevelopmentMeasurement(
    ...
    return iss.fit_iss_gain_target(measurements)


@pytest.mark.unit
def test_iss_observation_template_are_normalized_and_match_whitening_score() -> None:
    assert iss.CONTENT_ISS_METHOD_ID == "content_v6_detector_domain_iss_lf_adaptive_hf_v1"
    assert iss.CONTENT_ISS_EVALUATED_CANDIDATE_ID == (
    ...
    phi = iss.content_iss_observation(image, assets)
    template = iss.content_iss_template(key, assets)
    score = iss.score_content_iss_image(image, key, assets)
...
def test_fit_formula_uses_median_gain_rank28_and_exact_margin() -> None:
    ...
    bad = [iss.ISSDevelopmentMeasurement(0.0, 0.0, 0.0)] * 32
    with pytest.raises(ValueError, match="gain must be finite and positive"):
        iss.fit_iss_gain_target(bad)


@pytest.mark.unit
def test_asset_binary64_sidecar_and_beta_boundaries_fail_closed(tmp_path: Path) -> None:
    key = iss.derive_development_key(b"content-v6-root-key-material")
    asset = iss.build_iss_asset("1" * 40, key, _fit())
    ...
    path = tmp_path / f"{iss.ISS_ASSET_ROLE_ID}.json"
    sidecar = tmp_path / f"{iss.ISS_ASSET_ROLE_ID}.json.sha256"
    path.write_bytes(asset.json_bytes)
    digest = hashlib.sha256(asset.json_bytes).hexdigest()
    sidecar.write_text(f"{digest}  {path.name}\n", encoding="ascii")
    loaded = iss.load_iss_asset(path, sidecar)
    assert loaded == asset
    ...
    assert iss.iss_beta(low_host, loaded) == 1.0
    assert iss.iss_beta(middle_host, loaded) == pytest.approx(1.5)
    assert iss.iss_beta(high_host, loaded) == 2.0
    for invalid in (True, math.nan, math.inf):
        with pytest.raises((TypeError, ValueError)):
            iss.iss_beta(invalid, loaded)
    sidecar.write_text(f"{'0'*64}  {path.name}\n", encoding="ascii")
    with pytest.raises(ValueError, match="sidecar binding"):
        iss.load_iss_asset(path, sidecar)
    with pytest.raises(ValueError, match="rank-28 rule"):
        iss.build_iss_asset("1" * 40, key, iss.ISSFit(0.2, 0.4, 0.1))


@pytest.mark.unit
def test_frozen_iss_gain_target_asset_is_the_accepted_exact_pair() -> None:
    asset_path = _ROOT / iss.ISS_ASSET_REPO_PATH
    sidecar_path = _ROOT / iss.ISS_ASSET_SIDECAR_REPO_PATH
    raw = asset_path.read_bytes()
    sidecar = sidecar_path.read_bytes()
    assert len(raw) == 1152
    assert len(sidecar) == 101
    assert hashlib.sha256(raw).hexdigest() == iss.ISS_ASSET_SHA256
    assert hashlib.sha256(sidecar).hexdigest() == iss.ISS_ASSET_SIDECAR_SHA256
    assert sidecar == f"{iss.ISS_ASSET_SHA256}  {asset_path.name}\n".encode("ascii")
    asset = iss.load_frozen_content_iss_asset(_ROOT)
...
@pytest.mark.unit
def test_iss_beta_scales_only_lf_before_the_unchanged_common_projector(
    ...
    monkeypatch.setattr(iss, "_content_unweighted_branch_deltas", branch_deltas)
    allocation = unweighted.ContentAllocation(
        (1.0,) * 16, (1.0,) * 16, 0.4, 0.6, (0.1,) * 6
    )
    one, one_measurement = iss.embed_content_iss(
        base, b"key", object(), object(), allocation, 1.0
    )
    two, two_measurement = iss.embed_content_iss(
    ...
    for invalid in (0.999, 2.001, math.nan, True):
        with pytest.raises((TypeError, ValueError)):
            iss.embed_content_iss(base, b"key", object(), object(), allocation, invalid)
