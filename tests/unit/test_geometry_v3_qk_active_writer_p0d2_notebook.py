...
NOTEBOOK = (
    ...
    / "notebooks"
    / "geometry_v3_qk_active_writer_p0d2_colab.ipynb"
)
RUNNER = (
    Path(__file__).resolve().parents[2]
    / "experiments"
...
RUNNER_EXACT = "a27a940ae1ef4d1141925c6304caab55b89ec999"
...
def test_notebook_binds_exact_single_runner_control_and_drive_contract() -> None:
    ...
    assert f"P0D2_RUNNER_EXACT = '{RUNNER_EXACT}'" in code
    assert "git', 'checkout', '--detach', P0D2_RUNNER_EXACT" in code
    assert "resolved != P0D2_RUNNER_EXACT or dirty" in code
    assert "runner_path = checkout / RUNNER_RELATIVE_PATH" in code
    assert "'/content/drive/MyDrive/CEG-WM/Geometry-V3/P0D2'" in code
...
@pytest.mark.unit
def test_notebook_plan_is_one_fixed_execution_without_method_choices() -> None:
    code = "\n".join(_code_cells())
    plan_block = code[code.index("plan = {") : code.index("plan_file =")]
    assert set(
