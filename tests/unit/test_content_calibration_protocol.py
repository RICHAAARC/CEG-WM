...
from cegwm.protocol import content_calibration as calibration

_ROOT = Path(__file__).resolve().parents[2]


@pytest.mark.unit
def test_content_calibration_manifests_are_exact_ordered_unique_and_disjoint() -> None:
    contract = calibration.load_content_calibration_contract(_ROOT)
    ...
    ).hexdigest() == calibration.CONTENT_CALIBRATION_CALIBRATION_MANIFEST_SHA256
    assert hashlib.sha256(
        (_ROOT / "configs/content_chain/content_v9_clean_evaluation_v1.jsonl").read_bytes()
    ).hexdigest() == calibration.CONTENT_CALIBRATION_EVALUATION_MANIFEST_SHA256
...
@pytest.mark.unit
def test_content_calibration_phase1_identity_has_calibration_digest_but_no_fabricated_final_asset() -> None:
    contract = calibration.load_content_calibration_contract(_ROOT)
    assert contract.protocol_digest == calibration.CONTENT_CALIBRATION_CALIBRATION_PROTOCOL_DIGEST
    assert contract.config["protocol_id"] == calibration.CONTENT_CALIBRATION_CALIBRATION_PROTOCOL_ID
    assert contract.config["future_evaluation"]["protocol_id"] == (
        calibration.CONTENT_CALIBRATION_FUTURE_EVALUATION_PROTOCOL_ID
    )
    ...
    run_id = calibration.deterministic_calibration_run_id(contract.protocol_digest, "a" * 64)
    assert run_id == f"content-v9-calibration-{contract.protocol_digest[:12]}-{'a' * 12}"


@pytest.mark.unit
def test_content_calibration_manifest_drift_fails_closed(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="manifest bytes differ"):
        calibration.load_content_calibration_contract(tmp_path)
