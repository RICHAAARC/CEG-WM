...
def test_rotation_shape_dtype_mask_and_determinism(height: int, width: int) -> None:
    ...
    first = rotation_10_bicubic_reflect_center_crop(rgb, implementation_exact=IMPLEMENTATION_EXACT)
    second = rotation_10_bicubic_reflect_center_crop(rgb, implementation_exact=IMPLEMENTATION_EXACT)
    ...
    assert np.all(first.rgb[first.valid_mask == 1] == 255)  # black Pillow fill cannot leak into valid support
...
def test_positive_angle_is_visual_counter_clockwise_not_inverted() -> None:
    ...
    result = rotation_10_bicubic_reflect_center_crop(rgb, implementation_exact=IMPLEMENTATION_EXACT)
...
def test_reflected_rgb_and_zero_mask_are_separate() -> None:
    rgb = np.zeros((41, 73, 3), dtype=np.uint8)
    rgb[:, 0] = (17, 34, 51)
    result = rotation_10_bicubic_reflect_center_crop(rgb, implementation_exact=IMPLEMENTATION_EXACT)
...
def test_invalid_small_or_reflect_padding_inputs_fail_closed(shape: tuple[int, int, int]) -> None:
    with pytest.raises((TypeError, ValueError)):
        rotation_10_bicubic_reflect_center_crop(np.zeros(shape, dtype=np.uint8), implementation_exact=IMPLEMENTATION_EXACT)


@pytest.mark.unit
def test_non_rgb_or_non_uint8_input_fails_closed() -> None:
    with pytest.raises(TypeError):
        rotation_10_bicubic_reflect_center_crop(np.zeros((41, 73), dtype=np.uint8), implementation_exact=IMPLEMENTATION_EXACT)
    with pytest.raises(TypeError):
        rotation_10_bicubic_reflect_center_crop(np.zeros((41, 73, 3), dtype=np.float32), implementation_exact=IMPLEMENTATION_EXACT)
