...
from cegwm.method import content_weighted_joint as calibration


def _pairs() -> tuple[calibration.LFHFScorePair, ...]:
    return tuple(
        calibration.LFHFScorePair(
            -0.6 + index / 1000.0,
...
def _asset() -> calibration.WeightedJointAsset:
    return calibration.build_calibration_asset(
        producer_exact="1" * 40,
        protocol_digest="2" * 64,
        public_key_digest="3" * 64,
        fit=calibration.fit_weighted_joint_calibration(_pairs()),
    )
...
def test_binary64_fit_uses_exact_order_count_ddof1_and_paired_rho() -> None:
    pairs = _pairs()
    fit = calibration.fit_weighted_joint_calibration(pairs)
    ...
    with pytest.raises(ValueError, match="exactly 1056"):
        calibration.fit_weighted_joint_calibration(pairs[:-1])
    degenerate = (calibration.LFHFScorePair(0.0, 0.0),) * 1056
    with pytest.raises(ValueError, match="variances"):
        calibration.fit_weighted_joint_calibration(degenerate)
    with pytest.raises(TypeError):
        calibration.fit_weighted_joint_calibration((calibration.LFHFScorePair(True, 0.0),) * 1056)
...
def test_asset_is_stable_create_only_payload_with_exact_five_f64_values(tmp_path: Path) -> None:
    ...
    assert calibration.load_calibration_asset(path, sidecar) == asset
    sidecar.write_bytes(f"{'0' * 64}  {path.name}\n".encode("ascii"))
    with pytest.raises(ValueError, match="sidecar binding"):
        calibration.load_calibration_asset(path, sidecar)
...
def test_weights_apply_only_to_z_and_branch_diagnostics_cannot_veto_weighted_gates() -> None:
    ...
    assert calibration.weighted_joint_score(lf, hf, asset) == expected
    registered = calibration.LFHFScorePair(-0.1, 0.9)
    wrong = tuple(calibration.LFHFScorePair(0.8, -0.8) for _ in range(16))
    null = calibration.LFHFScorePair(0.7, -0.7)
    evidence = calibration.weighted_gate_evidence(registered, wrong, null, asset)
    ...
    with pytest.raises(TypeError):
        calibration.weighted_joint_score(True, 0.0, asset)
...
def test_accepted_asset_golden_values_and_weighted_score_are_not_raw_minimum() -> None:
    ...
    asset = calibration.load_calibration_asset(path, path.with_name(f"{path.name}.sha256"))
    fit = asset.fit
    assert fit == calibration.CalibrationFit(
    ...
    score = calibration.weighted_joint_score(0.01, 0.02, asset)
