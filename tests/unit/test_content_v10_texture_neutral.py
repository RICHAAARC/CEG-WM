...
class ContentV10Tests(unittest.TestCase):
 def test_contract_and_v10_only_asset(self):
  ...
  with tempfile.TemporaryDirectory() as d:
   p=Path(d)/"asset.json"; value={"schema_version":1,"method_id":METHOD_ID,"asset_role_id":"content_v10_weighted_joint_calibration","lf_weight":.25,"hf_weight":.75,"lf_scorer_id":"lf","hf_scorer_id":"hf","calibration_manifest_digest":"a"*64,"mu_lf":0.,"sigma_lf":1.,"mu_hf":0.,"sigma_hf":2.,"rho":0.}; raw=json.dumps(value,sort_keys=True,separators=(",",":")).encode(); p.write_bytes(raw); p.with_name("asset.json.sha256").write_bytes((hashlib.sha256(raw).hexdigest()+"  asset.json\n").encode("ascii"))
   asset=load_independent_calibration_asset(p,p.with_name("asset.json.sha256")); self.assertAlmostEqual(weighted_joint_v10(1.,2.,asset),1.0/(.25**2+.75**2)**.5)
