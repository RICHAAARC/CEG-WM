...
# Unit coverage for the content-adaptive protocol.
...
from cegwm.protocol.content_adaptive import load_content_adaptive_protocol
...
@pytest.mark.unit
def test_adaptive_protocol_freezes_64_probe_six_effect_and_fresh_fixed_denominator_identity() -> None:
    protocol = load_content_adaptive_protocol(_CONFIG, _ROSTER)
...
@pytest.mark.unit
def test_adaptive_protocol_rejects_formula_identity_detector_or_private_export_drift(tmp_path: Path) -> None:
    mutations = (
    ...
    for section, field, value, message in mutations:
        ...
        with pytest.raises(ValueError, match=message):
            load_content_adaptive_protocol(modified, _ROSTER)


@pytest.mark.unit
def test_adaptive_protocol_detection_is_blind_and_private_maps_are_never_exported() -> None:
    protocol = load_content_adaptive_protocol(_CONFIG, _ROSTER)
...
@pytest.mark.unit
def test_adaptive_loader_uses_only_the_explicit_config_and_ordered_roster(tmp_path: Path) -> None:
    ...
    protocol = load_content_adaptive_protocol(config, roster)
    assert len(protocol.roster) == 8


@pytest.mark.unit
def test_adaptive_loader_rejects_any_ordered_roster_identity_drift(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="frozen eight units"):
        load_content_adaptive_protocol(_CONFIG, drifted)
