...
@pytest.mark.unit
def test_stage_a_attack_complementarity_notebook_has_one_thin_handoff_path() -> None:
    ...
    combined = "\n".join(sources)
    assert "refs/heads/stage-a-hf-lf-attack-complementarity" in combined
    assert "/content/drive/MyDrive/CEG-WM/stage_a_hf_lf_attack_complementarity" in combined
    assert combined.count("hlfac-[0-9a-f]{24}") == 2
