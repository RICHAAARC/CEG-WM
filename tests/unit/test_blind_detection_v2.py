...
import pytest
from cegwm.runtime import blind_detection_v2 as prototype
...
def test_finite_search_keeps_all_scores_and_never_threshold_shortcuts(monkeypatch):
    ...
    result=prototype._search_candidates(source,scorer,plan=plan)
...
def test_failed_and_nonfinite_scores_are_retained_without_a_positive():
    ...
    def scorer(image):
        ...
        return float("nan")
    result=prototype._search_candidates(Image.new("RGB",(512,512)),scorer,
...
def test_off_grid_asymmetric_image_refinement_is_sampled_from_original():
    ...
    result=prototype._search_candidates(observed,fixture_score,
...
def test_default_budget_and_unavailable_h_do_not_remove_rotation_candidates():
    ...
    result=prototype._search_candidates(Image.new("RGB",(512,512)),lambda image:0.,lambda image:unsupported,plan)
...
def test_inconsistent_geometry_never_claims_complete(status):
    ...
    result=prototype._search_candidates(Image.new("RGB",(512,512)),lambda image:0.,lambda image:inconsistent,
