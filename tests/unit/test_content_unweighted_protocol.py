...
# Unit coverage for the content-unweighted protocol.
...
@pytest.mark.unit
def test_content_unweighted_protocol_has_distinct_method_protocol_digest_record_and_run_inputs() -> None:
    protocol = load_content_unweighted_clean_protocol(_CONFIG, _ROSTER)
    adaptive = load_content_adaptive_protocol(_ADAPTIVE_CONFIG, _ROSTER)
    assert protocol.protocol_id == CONTENT_UNWEIGHTED_PROTOCOL_ID
    assert protocol.protocol_digest == CONTENT_UNWEIGHTED_PROTOCOL_DIGEST
    assert protocol.protocol_digest != adaptive.protocol_digest
    ...
    assert identities["hf_adaptive_embedding_transform_id"] == (
        adaptive.config["method_identities"]["hf_adaptive_embedding_transform_id"]
    )
...
@pytest.mark.unit
def test_content_unweighted_reuses_exact_roster_science_key_and_blind_detection_contracts() -> None:
    protocol = load_content_unweighted_clean_protocol(_CONFIG, _ROSTER)
    adaptive = load_content_adaptive_protocol(_ADAPTIVE_CONFIG, _ROSTER)
    assert hashlib.sha256(_ROSTER.read_bytes()).hexdigest() == CONTENT_UNWEIGHTED_ROSTER_SHA256
    assert protocol.config["execution_flow"]["formal_roster_sha256"] == CONTENT_UNWEIGHTED_ROSTER_SHA256
    assert protocol.roster == adaptive.roster
    assert tuple((unit.prompt, unit.seed) for unit in protocol.roster) == tuple(
        (unit.prompt, unit.seed) for unit in adaptive.roster
    )
    assert protocol.config["decision_rule"] == adaptive.config["decision_rule"]
    assert protocol.config["keying"] == adaptive.config["keying"]
    assert protocol.config["keying"]["wrong_key_count"] == 16
    assert protocol.config["detection_access"] == adaptive.config["detection_access"]
...
@pytest.mark.unit
def test_content_unweighted_keeps_runtime_asset_contract_separate_from_method_version() -> None:
    protocol = load_content_unweighted_clean_protocol(_CONFIG, _ROSTER)
...
@pytest.mark.unit
def test_provenance_exclusions_do_not_define_unweighted_method_identity() -> None:
    protocol = load_content_unweighted_clean_protocol(_CONFIG, _ROSTER)
...
def test_content_unweighted_loader_fails_closed_on_identity_or_science_drift(
    tmp_path: Path,
    section: str,
    field: str,
    value: object,
    ...
    with pytest.raises(ValueError, match=message):
        load_content_unweighted_clean_protocol(modified, _ROSTER)
