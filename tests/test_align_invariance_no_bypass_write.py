...
def test_align_invariance_extractor_must_not_write_records_or_artifacts(monkeypatch: MonkeyPatch) -> None:
    """
    功能：对齐与评分提取器不得直接调用 records_io 写盘入口。

    Aligner extractor must not directly call records_io write APIs.
    ...
    def _forbidden_call(*args: Any, **kwargs: Any) -> None:
        called["count"] += 1
        raise AssertionError("records_io write API must not be called by align extractor")
    ...
    latents = np.random.RandomState(5).randn(1, 4, 8, 8).astype(np.float32)
