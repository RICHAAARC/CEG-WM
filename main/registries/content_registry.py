...
功能说明：
- 定义了 content extractor（内容提取器）、子空间规划器（subspace planner）等模块的运行时注册表。
- 仅注册正式实现，不含测试脚手架或基线。
"""
...
def _build_low_freq_template_codec(cfg: Dict[str, Any]) -> LowFreqTemplateCodec:
    """
    功能：构造 LF 低频模板编码器实现。

    Build LF low-frequency template codec (additive pseudogaussian template injection).
    ...
    impl_digest = _derive_impl_digest(LOW_FREQ_TEMPLATE_CODEC_ID, impl_version)
...
def _build_unified_content_extractor(cfg: Dict[str, Any]) -> UnifiedContentExtractor:
    ...
    impl_digest = _derive_impl_digest(UNIFIED_CONTENT_EXTRACTOR_ID, impl_version)
    return UnifiedContentExtractor(UNIFIED_CONTENT_EXTRACTOR_ID, impl_version, impl_digest)


def _build_subspace_planner(cfg: Dict[str, Any]) -> SubspacePlanner:
    """
    功能：构造子空间规划器实现。

    Build subspace planner (with semantic domain annotations).

    Args:
        cfg: Config mapping.

    Returns:
        SubspacePlanner instance.
    ...
    return SubspacePlanner(SUBSPACE_PLANNER_ID, impl_version, impl_digest)
...
_CONTENT_REGISTRY.register_factory(
    LOW_FREQ_TEMPLATE_CODEC_ID,
    _build_low_freq_template_codec,
    capabilities=ImplCapabilities(
        supports_batching=False,
        requires_cuda=False,
        supports_deterministic=True,
